...
const defaultEvaluator =
          {
              hydrate:
                  function (items: RawSpwConstruct[] | undefined) {
                      return items;
                  },
              stringify:
                  function (items) {
                      return Array.from(items ?? []).filter(Boolean).filter(Boolean).join('');
                  },
          } as ComponentEvaluatorObject;


function makeSelector(override: Partial<ComponentDescription<any, any[], any, any>> & { name: string }) {
...
function makeEvaluators(override: Partial<ComponentDescription<any, any[], any, any>> & { name: string }) {
...
export default function LanguageComponentConstructor(config: LanguageComponentProps): ComponentDescription {
