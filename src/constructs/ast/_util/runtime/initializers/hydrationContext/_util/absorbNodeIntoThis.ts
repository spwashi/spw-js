 ...
 * @param node
 */
export function absorbNodeIntoThis(this: { registers: RuntimeRegisters }, node: AbsorbInput): AbsorbOutput | null {
    ...
    const keys =
              registers.indexed.set(
                  key,
                  registers.indexed.get(key) ?? new Register(),
              );

    const keyRegister =
              keys.get(key) as Register<typeof node>;
