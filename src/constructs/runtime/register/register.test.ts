...
describe('Registers', () => {
    ...
    it('Can list all items registered', () => {
        ...
        const inputItems =
                  Array(faker.datatype.number(MAX_ITEMS)).map(() => faker.datatype.string());

        // output

        const outputRegister =
                  new Register(inputItems);

        const outputFlat =
                  outputRegister.flat;

        expect(outputFlat.length)
            .toEqual(inputItems.length);
    ...
    it('Responds to a "memory" setting by limiting output to that number of entries', () => {
        ...
        const inputItems  =
                  Array(faker.datatype.number(MAX_ITEMS))
        ...
        const outputRegister =
                  new Register(inputItems, {memory: inputMemory});
        const outputFlat     =
                  outputRegister.flat;
        ...
        const reversedSeed = [...inputItems].reverse();
        outputFlat
            .map((item, i) => expect(item).toEqual(reversedSeed[i]))
    });
})