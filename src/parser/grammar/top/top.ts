...
import {SpwContainerNode} from '@constructs/ast/nodes/containers/_abstract/container';
...
const fragments =
          _.flatten(
              ([Domain] as typeof SpwContainerNode[]).map((Comp) => {
...
const pattern =
          sequenceOf([
                         oneOrMoreOf(anyOf(topRuleOptions)),
                     ]);
...
export const allowedStartRules =
                 [
                     ruleName,
                 ];
