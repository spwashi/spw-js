// language=JavaScript
/* eslint-disable */
// @ts-nocheck
// noinspection UnnecessaryLocalVariableJS
const generatedParser =
  /*
   * Generated by PEG.js 0.10.0.
   *
   * http://pegjs.org/
   */
  (function () {
    'use strict';

    function peg$subclass(child, parent) {
      function ctor() {
        this.constructor = child;
      }
      ctor.prototype = parent.prototype;
      child.prototype = new ctor();
    }

    function peg$SyntaxError(message, expected, found, location) {
      this.message = message;
      this.expected = expected;
      this.found = found;
      this.location = location;
      this.name = 'SyntaxError';

      if (typeof Error.captureStackTrace === 'function') {
        Error.captureStackTrace(this, peg$SyntaxError);
      }
    }

    peg$subclass(peg$SyntaxError, Error);

    peg$SyntaxError.buildMessage = function (expected, found) {
      var DESCRIBE_EXPECTATION_FNS = {
        literal: function (expectation) {
          return '"' + literalEscape(expectation.text) + '"';
        },

        class: function (expectation) {
          var escapedParts = '',
            i;

          for (i = 0; i < expectation.parts.length; i++) {
            escapedParts +=
              expectation.parts[i] instanceof Array
                ? classEscape(expectation.parts[i][0]) + '-' + classEscape(expectation.parts[i][1])
                : classEscape(expectation.parts[i]);
          }

          return '[' + (expectation.inverted ? '^' : '') + escapedParts + ']';
        },

        any: function (expectation) {
          return 'any character';
        },

        end: function (expectation) {
          return 'end of input';
        },

        other: function (expectation) {
          return expectation.description;
        },
      };

      function hex(ch) {
        return ch.charCodeAt(0).toString(16).toUpperCase();
      }

      function literalEscape(s) {
        return s
          .replace(/\\/g, '\\\\')
          .replace(/"/g, '\\"')
          .replace(/\0/g, '\\0')
          .replace(/\t/g, '\\t')
          .replace(/\n/g, '\\n')
          .replace(/\r/g, '\\r')
          .replace(/[\x00-\x0F]/g, function (ch) {
            return '\\x0' + hex(ch);
          })
          .replace(/[\x10-\x1F\x7F-\x9F]/g, function (ch) {
            return '\\x' + hex(ch);
          });
      }

      function classEscape(s) {
        return s
          .replace(/\\/g, '\\\\')
          .replace(/\]/g, '\\]')
          .replace(/\^/g, '\\^')
          .replace(/-/g, '\\-')
          .replace(/\0/g, '\\0')
          .replace(/\t/g, '\\t')
          .replace(/\n/g, '\\n')
          .replace(/\r/g, '\\r')
          .replace(/[\x00-\x0F]/g, function (ch) {
            return '\\x0' + hex(ch);
          })
          .replace(/[\x10-\x1F\x7F-\x9F]/g, function (ch) {
            return '\\x' + hex(ch);
          });
      }

      function describeExpectation(expectation) {
        return DESCRIBE_EXPECTATION_FNS[expectation.type](expectation);
      }

      function describeExpected(expected) {
        var descriptions = new Array(expected.length),
          i,
          j;

        for (i = 0; i < expected.length; i++) {
          descriptions[i] = describeExpectation(expected[i]);
        }

        descriptions.sort();

        if (descriptions.length > 0) {
          for (i = 1, j = 1; i < descriptions.length; i++) {
            if (descriptions[i - 1] !== descriptions[i]) {
              descriptions[j] = descriptions[i];
              j++;
            }
          }
          descriptions.length = j;
        }

        switch (descriptions.length) {
          case 1:
            return descriptions[0];

          case 2:
            return descriptions[0] + ' or ' + descriptions[1];

          default:
            return (
              descriptions.slice(0, -1).join(', ') + ', or ' + descriptions[descriptions.length - 1]
            );
        }
      }

      function describeFound(found) {
        return found ? '"' + literalEscape(found) + '"' : 'end of input';
      }

      return 'Expected ' + describeExpected(expected) + ' but ' + describeFound(found) + ' found.';
    };

    function peg$parse(input, options) {
      options = options !== void 0 ? options : {};

      var peg$FAILED = {},
        peg$startRuleFunctions = { Top: peg$parseTop },
        peg$startRuleFunction = peg$parseTop,
        peg$c0 = peg$otherExpectation('Top'),
        peg$c1 = function () {
          return null;
        },
        peg$c2 = /^[\n]/,
        peg$c3 = peg$classExpectation(['\n'], false, false),
        peg$c4 = function (body) {
          return null;
        },
        peg$c5 = function (body) {
          const items = Array.isArray(body)
            ? body
                .map((item) => (item && item.kind ? item : undefined))
                .filter((item) => typeof item !== 'undefined')
            : body;
          return items.length === 1 ? items[0] : items;
        },
        peg$c6 = peg$otherExpectation('UnicodeWithoutQuotes'),
        peg$c7 = /^[\-a-zA-Z \t']/,
        peg$c8 = peg$classExpectation(['-', ['a', 'z'], ['A', 'Z'], ' ', '\t', "'"], false, false),
        peg$c9 = /^[ -!,#-Y,a-\u26FF]/,
        peg$c10 = peg$classExpectation(
          [[' ', '!'], ',', ['#', 'Y'], ',', ['a', '\u26FF']],
          false,
          false,
        ),
        peg$c11 = peg$otherExpectation('Space'),
        peg$c12 = /^[\t]/,
        peg$c13 = peg$classExpectation(['\t'], false, false),
        peg$c14 = ' ',
        peg$c15 = peg$literalExpectation(' ', false),
        peg$c16 = function (newline) {
          return newline;
        },
        peg$c17 = function (newlines) {
          return constructs.space();
        },
        peg$c18 = peg$otherExpectation('Node'),
        peg$c19 = peg$otherExpectation('AnchorNode'),
        peg$c20 = /^[a-zA-Z0-9]/,
        peg$c21 = peg$classExpectation(
          [
            ['a', 'z'],
            ['A', 'Z'],
            ['0', '9'],
          ],
          false,
          false,
        ),
        peg$c22 = '_',
        peg$c23 = peg$literalExpectation('_', false),
        peg$c24 = function (chars) {
          return chars.join('');
        },
        peg$c25 = function (tail) {
          return [...tail].join('');
        },
        peg$c26 = /^[a-zA-Z]/,
        peg$c27 = peg$classExpectation(
          [
            ['a', 'z'],
            ['A', 'Z'],
          ],
          false,
          false,
        ),
        peg$c28 = function (head, tail) {
          const characters = [...head, ...tail];
          return characters.join('');
        },
        peg$c29 = function (anchor) {
          return toConstruct({
            kind: 'anchor',
            label: anchor,
          });
        },
        peg$c30 = peg$otherExpectation('EmbedmentNode'),
        peg$c31 = /^[`]/,
        peg$c32 = peg$classExpectation(['`'], false, false),
        peg$c33 = /^[\\]/,
        peg$c34 = peg$classExpectation(['\\'], false, false),
        peg$c35 = /^["]/,
        peg$c36 = peg$classExpectation(['"'], false, false),
        peg$c37 = /^[']/,
        peg$c38 = peg$classExpectation(["'"], false, false),
        peg$c39 = function (body) {
          return body.join('');
        },
        peg$c40 = function (embedment) {
          return toConstruct({
            kind: 'embedment',
            open: '`',
            body: embedment,
            close: '`',
          });
        },
        peg$c41 = peg$otherExpectation('NumberNode'),
        peg$c42 = /^[0-9]/,
        peg$c43 = peg$classExpectation([['0', '9']], false, false),
        peg$c44 = function (num) {
          return toConstruct({
            kind: 'number',
            value: parseInt(num.join('')),
          });
        },
        peg$c45 = peg$otherExpectation('PhraseNode'),
        peg$c46 = function (head, anchor) {
          return anchor;
        },
        peg$c47 = function (head, tail) {
          const items = [head, ...tail];
          return items;
        },
        peg$c48 = function (phrase) {
          var makeArray = (c) => (Array.isArray(c) ? c : [c]);
          /** @var {Array} phrase*/
          const _phrase = phrase;
          const p = _phrase.reduce((p, c) => [...p, ...makeArray(c)], []);
          return toConstruct({
            kind: 'phrase',
            items: p,
          });
        },
        peg$c49 = peg$otherExpectation('Scalar'),
        peg$c50 = peg$otherExpectation('StringNode'),
        peg$c51 = '\\',
        peg$c52 = peg$literalExpectation('\\', false),
        peg$c53 = function () {
          return '"';
        },
        peg$c54 = function (string) {
          return toConstruct({
            kind: 'string',
            open: '"',
            body: string,
            close: '"',
          });
        },
        peg$c55 = peg$otherExpectation('Operator'),
        peg$c56 = peg$otherExpectation('AggregationOperator'),
        peg$c57 = '+',
        peg$c58 = peg$literalExpectation('+', false),
        peg$c59 = function (token, label) {
          return { token: token, label: label };
        },
        peg$c60 = function (_operatorComponents) {
          return toConstruct({
            kind: 'aggregation',
            ..._operatorComponents,
          });
        },
        peg$c61 = peg$otherExpectation('AscentOperator'),
        peg$c62 = '^',
        peg$c63 = peg$literalExpectation('^', false),
        peg$c64 = function (_operatorComponents) {
          return toConstruct({
            kind: 'ascent',
            ..._operatorComponents,
          });
        },
        peg$c65 = peg$otherExpectation('BindingOperator'),
        peg$c66 = ':',
        peg$c67 = peg$literalExpectation(':', false),
        peg$c68 = function (_operatorComponents) {
          return toConstruct({
            kind: 'binding',
            ..._operatorComponents,
          });
        },
        peg$c69 = peg$otherExpectation('BranchOperator'),
        peg$c70 = '|',
        peg$c71 = peg$literalExpectation('|', false),
        peg$c72 = function (_operatorComponents) {
          return toConstruct({
            kind: 'branch',
            ..._operatorComponents,
          });
        },
        peg$c73 = peg$otherExpectation('ChannelOperator'),
        peg$c74 = '#',
        peg$c75 = peg$literalExpectation('#', false),
        peg$c76 = function (_operatorComponents) {
          return toConstruct({
            kind: 'channel',
            ..._operatorComponents,
          });
        },
        peg$c77 = peg$otherExpectation('DescentOperator'),
        peg$c78 = '.',
        peg$c79 = peg$literalExpectation('.', false),
        peg$c80 = function (_operatorComponents) {
          return toConstruct({
            kind: 'descent',
            ..._operatorComponents,
          });
        },
        peg$c81 = peg$otherExpectation('DirectionOperator'),
        peg$c82 = '.-',
        peg$c83 = peg$literalExpectation('.-', false),
        peg$c84 = function (_operatorComponents) {
          return toConstruct({
            kind: 'direction',
            ..._operatorComponents,
          });
        },
        peg$c85 = peg$otherExpectation('EvaluationOperator'),
        peg$c86 = '?',
        peg$c87 = peg$literalExpectation('?', false),
        peg$c88 = function (_operatorComponents) {
          return toConstruct({
            kind: 'evaluation',
            ..._operatorComponents,
          });
        },
        peg$c89 = peg$otherExpectation('InvocationOperator'),
        peg$c90 = '~',
        peg$c91 = peg$literalExpectation('~', false),
        peg$c92 = function (_operatorComponents) {
          return toConstruct({
            kind: 'invocation',
            ..._operatorComponents,
          });
        },
        peg$c93 = peg$otherExpectation('PerformanceOperator'),
        peg$c94 = '!',
        peg$c95 = peg$literalExpectation('!', false),
        peg$c96 = function (_operatorComponents) {
          return toConstruct({
            kind: 'performance',
            ..._operatorComponents,
          });
        },
        peg$c97 = peg$otherExpectation('PerspectiveOperator'),
        peg$c98 = '@',
        peg$c99 = peg$literalExpectation('@', false),
        peg$c100 = function (_operatorComponents) {
          return toConstruct({
            kind: 'perspective',
            ..._operatorComponents,
          });
        },
        peg$c101 = peg$otherExpectation('RangeOperator'),
        peg$c102 = '..',
        peg$c103 = peg$literalExpectation('..', false),
        peg$c104 = function (_operatorComponents) {
          return toConstruct({
            kind: 'range',
            ..._operatorComponents,
          });
        },
        peg$c105 = peg$otherExpectation('ReductionOperator'),
        peg$c106 = '-',
        peg$c107 = peg$literalExpectation('-', false),
        peg$c108 = function (_operatorComponents) {
          return toConstruct({
            kind: 'reduction',
            ..._operatorComponents,
          });
        },
        peg$c109 = peg$otherExpectation('ReferenceOperator'),
        peg$c110 = '&',
        peg$c111 = peg$literalExpectation('&', false),
        peg$c112 = function (_operatorComponents) {
          return toConstruct({
            kind: 'reference',
            ..._operatorComponents,
          });
        },
        peg$c113 = peg$otherExpectation('SpreadOperator'),
        peg$c114 = '...',
        peg$c115 = peg$literalExpectation('...', false),
        peg$c116 = function (_operatorComponents) {
          return toConstruct({
            kind: 'spread',
            ..._operatorComponents,
          });
        },
        peg$c117 = peg$otherExpectation('TransformationOperator'),
        peg$c118 = '=>',
        peg$c119 = peg$literalExpectation('=>', false),
        peg$c120 = function (_operatorComponents) {
          return toConstruct({
            kind: 'transformation',
            ..._operatorComponents,
          });
        },
        peg$c121 = peg$otherExpectation('ValueOperator'),
        peg$c122 = '*',
        peg$c123 = peg$literalExpectation('*', false),
        peg$c124 = function (_operatorComponents) {
          return toConstruct({
            kind: 'value',
            ..._operatorComponents,
          });
        },
        peg$c125 = peg$otherExpectation('BlockDelimiter'),
        peg$c126 = ';',
        peg$c127 = peg$literalExpectation(';', false),
        peg$c128 = function (_operatorComponents) {
          return toConstruct({
            kind: 'block_delimiter',
            ..._operatorComponents,
          });
        },
        peg$c129 = peg$otherExpectation('CommonDelimiter'),
        peg$c130 = ',',
        peg$c131 = peg$literalExpectation(',', false),
        peg$c132 = function (_operatorComponents) {
          return toConstruct({
            kind: 'common_delimiter',
            ..._operatorComponents,
          });
        },
        peg$c133 = peg$otherExpectation('NodeDelimiter'),
        peg$c134 = function (_operatorComponents) {
          return toConstruct({
            kind: 'node_delimiter',
            ..._operatorComponents,
          });
        },
        peg$c135 = peg$otherExpectation('Container'),
        peg$c136 = peg$otherExpectation('DomainOpen'),
        peg$c137 = '{',
        peg$c138 = peg$literalExpectation('{', false),
        peg$c139 = function (token, anchor, description) {
          return { anchor: anchor, description: description };
        },
        peg$c140 = function (token, node) {
          return null;
        },
        peg$c141 = function (token, node) {
          return toConstruct({
            token: token,
            position: 'open',
            label: node.anchor,
            description: node.description,
            kind: 'domain_scheme',
          });
        },
        peg$c142 = function (tok) {
          return toConstruct({ token: tok, position: 'open', kind: 'domain_scheme' });
        },
        peg$c143 = peg$otherExpectation('DomainClose'),
        peg$c144 = '}',
        peg$c145 = peg$literalExpectation('}', false),
        peg$c146 = function (token, node) {
          return toConstruct({
            token: token,
            position: 'close',
            label: node,
            kind: 'domain_identity',
          });
        },
        peg$c147 = function (node, token) {
          return toConstruct({
            token: token,
            position: 'close',
            label: node,
            kind: 'domain_identity',
          });
        },
        peg$c148 = function (tok) {
          return toConstruct({ token: tok, position: 'close', kind: 'domain_identity' });
        },
        peg$c149 = peg$otherExpectation('DomainBody'),
        peg$c150 = function (leadingspace, expression, trailingspace) {
          return expression;
        },
        peg$c151 = peg$otherExpectation('Domain'),
        peg$c152 = function (open) {
          return null;
        },
        peg$c153 = function (open, underscore) {
          return underscore;
        },
        peg$c154 = function (open, close) {
          return { open: open, close: close };
        },
        peg$c155 = function (open, body, close) {
          return { open: open, body: body, close: close };
        },
        peg$c156 = function (container) {
          return toConstruct({
            kind: 'domain',
            open: container.open,
            body: container.body,
            close: container.close,
          });
        },
        peg$c157 = peg$otherExpectation('EssenceOpen'),
        peg$c158 = '[',
        peg$c159 = peg$literalExpectation('[', false),
        peg$c160 = function (token, node) {
          return toConstruct({
            token: token,
            position: 'open',
            label: node.anchor,
            description: node.description,
            kind: 'essence_scheme',
          });
        },
        peg$c161 = function (tok) {
          return toConstruct({ token: tok, position: 'open', kind: 'essence_scheme' });
        },
        peg$c162 = peg$otherExpectation('EssenceClose'),
        peg$c163 = ']',
        peg$c164 = peg$literalExpectation(']', false),
        peg$c165 = function (token, node) {
          return toConstruct({
            token: token,
            position: 'close',
            label: node,
            kind: 'essence_identity',
          });
        },
        peg$c166 = function (node, token) {
          return toConstruct({
            token: token,
            position: 'close',
            label: node,
            kind: 'essence_identity',
          });
        },
        peg$c167 = function (tok) {
          return toConstruct({ token: tok, position: 'close', kind: 'essence_identity' });
        },
        peg$c168 = peg$otherExpectation('EssenceBody'),
        peg$c169 = peg$otherExpectation('Essence'),
        peg$c170 = function (container) {
          return toConstruct({
            kind: 'essence',
            open: container.open,
            body: container.body,
            close: container.close,
          });
        },
        peg$c171 = peg$otherExpectation('ConceptOpen'),
        peg$c172 = '<',
        peg$c173 = peg$literalExpectation('<', false),
        peg$c174 = function (token, node) {
          return toConstruct({
            token: token,
            position: 'open',
            label: node.anchor,
            description: node.description,
            kind: 'concept_scheme',
          });
        },
        peg$c175 = function (tok) {
          return toConstruct({ token: tok, position: 'open', kind: 'concept_scheme' });
        },
        peg$c176 = peg$otherExpectation('ConceptClose'),
        peg$c177 = '>',
        peg$c178 = peg$literalExpectation('>', false),
        peg$c179 = function (token, node) {
          return toConstruct({
            token: token,
            position: 'close',
            label: node,
            kind: 'concept_identity',
          });
        },
        peg$c180 = function (node, token) {
          return toConstruct({
            token: token,
            position: 'close',
            label: node,
            kind: 'concept_identity',
          });
        },
        peg$c181 = function (tok) {
          return toConstruct({ token: tok, position: 'close', kind: 'concept_identity' });
        },
        peg$c182 = peg$otherExpectation('ConceptBody'),
        peg$c183 = peg$otherExpectation('Concept'),
        peg$c184 = function (container) {
          return toConstruct({
            kind: 'concept',
            open: container.open,
            body: container.body,
            close: container.close,
          });
        },
        peg$c185 = peg$otherExpectation('LocationOpen'),
        peg$c186 = '(',
        peg$c187 = peg$literalExpectation('(', false),
        peg$c188 = function (token, node) {
          return toConstruct({
            token: token,
            position: 'open',
            label: node.anchor,
            description: node.description,
            kind: 'location_scheme',
          });
        },
        peg$c189 = function (tok) {
          return toConstruct({ token: tok, position: 'open', kind: 'location_scheme' });
        },
        peg$c190 = peg$otherExpectation('LocationClose'),
        peg$c191 = ')',
        peg$c192 = peg$literalExpectation(')', false),
        peg$c193 = function (token, node) {
          return toConstruct({
            token: token,
            position: 'close',
            label: node,
            kind: 'location_identity',
          });
        },
        peg$c194 = function (node, token) {
          return toConstruct({
            token: token,
            position: 'close',
            label: node,
            kind: 'location_identity',
          });
        },
        peg$c195 = function (tok) {
          return toConstruct({ token: tok, position: 'close', kind: 'location_identity' });
        },
        peg$c196 = peg$otherExpectation('LocationBody'),
        peg$c197 = peg$otherExpectation('Location'),
        peg$c198 = function (container) {
          return toConstruct({
            kind: 'location',
            open: container.open,
            body: container.body,
            close: container.close,
          });
        },
        peg$c199 = peg$otherExpectation('Expression'),
        peg$c200 = peg$otherExpectation('BlockExpression'),
        peg$c201 = function (expression) {
          return null;
        },
        peg$c202 = function (expression, delimiter) {
          return null;
        },
        peg$c203 = function (expression, delimiter) {
          return expression;
        },
        peg$c204 = function (head) {
          return null;
        },
        peg$c205 = function (head, tail) {
          return [...head, tail];
        },
        peg$c206 = function (head) {
          return [...head];
        },
        peg$c207 = function (tail) {
          return [tail];
        },
        peg$c208 = function (items) {
          const block = {
            kind: 'block',
            items: typeof items !== 'undefined' ? items : undefined,
          };
          return toConstruct(block);
        },
        peg$c209 = peg$otherExpectation('InstanceExpression'),
        peg$c210 = function (entity, space, behavior) {
          const expression = {
            kind: 'instance_expression',
            entity: entity,
            behavior: behavior,
          };
          return toConstruct(expression);
        },
        peg$c211 = peg$otherExpectation('BehaviorExpression'),
        peg$c212 = function (address, domain, essence) {
          const expression = {
            kind: 'behavior_expression',
            domain: 'undefined' != typeof domain ? domain : void 0,
            essence: 'undefined' != typeof essence ? essence : void 0,
            address: 'undefined' != typeof address ? address : void 0,
          };
          return toConstruct(expression);
        },
        peg$c213 = function (domain, essence) {
          const expression = {
            kind: 'behavior_expression',
            domain: 'undefined' != typeof domain ? domain : void 0,
            essence: 'undefined' != typeof essence ? essence : void 0,
            address: 'undefined' != typeof address ? address : void 0,
          };
          return toConstruct(expression);
        },
        peg$c214 = peg$otherExpectation('EntityExpression'),
        peg$c215 = function (concept, space, anchor) {
          const expression = {
            kind: 'entity_expression',
            anchor: anchor,
            concept: 'undefined' != typeof concept ? concept : void 0,
          };
          return toConstruct(expression);
        },
        peg$c216 = function (anchor) {
          const expression = {
            kind: 'entity_expression',
            anchor: anchor,
            concept: 'undefined' != typeof concept ? concept : void 0,
          };
          return toConstruct(expression);
        },
        peg$c217 = peg$otherExpectation('LocatedConceptExpression'),
        peg$c218 = function (address, concept) {
          const expression = {
            kind: 'located_concept_expression',
            address: typeof address !== 'undefined' ? address : undefined,
            concept: typeof concept !== 'undefined' ? concept : undefined,
          };
          return toConstruct(expression);
        },
        peg$c219 = peg$otherExpectation('LocatedDomainExpression'),
        peg$c220 = function (address, domain) {
          const expression = {
            kind: 'located_domain_expression',
            address: address,
            domain: domain,
          };
          return toConstruct(expression);
        },
        peg$c221 = peg$otherExpectation('LocatedEntityExpression'),
        peg$c222 = function (address, entity) {
          const expression = {
            kind: 'located_entity_expression',
            address: typeof address !== 'undefined' ? address : undefined,
            entity: typeof entity !== 'undefined' ? entity : undefined,
          };
          return toConstruct(expression);
        },
        peg$c223 = peg$otherExpectation('LocatedEssenceExpression'),
        peg$c224 = function (address, essence) {
          const expression = {
            kind: 'located_essence_expression',
            address: typeof address !== 'undefined' ? address : undefined,
            essence: typeof essence !== 'undefined' ? essence : undefined,
          };
          return toConstruct(expression);
        },
        peg$c225 = peg$otherExpectation('SequenceExpression'),
        peg$c226 = peg$otherExpectation('CommonExpression'),
        peg$c227 = function (head, operator) {
          return null;
        },
        peg$c228 = function (head, operator, item) {
          return null;
        },
        peg$c229 = function (head, operator, item) {
          return toConstruct({ kind: 'common_tail', operator: operator, item: item });
        },
        peg$c230 = function (head, tail) {
          return toConstruct({
            kind: 'common_expression',
            head,
            tail,
          });
        },
        peg$c231 = peg$otherExpectation('PhraseExpression'),
        peg$c232 = function (head, tail) {
          return tail;
        },
        peg$c233 = function (head, tail) {
          const phrase = {
            kind: 'phrase_expression',
            items: [head, ...tail],
          };

          return toConstruct(phrase);
        },
        peg$c234 = peg$otherExpectation('AggregationExpression'),
        peg$c235 = function (head, operator, item) {
          return toConstruct({
            kind: 'prefixed_aggregation_expression',
            operator: operator,
            item: item,
          });
        },
        peg$c236 = function (head, tail) {
          return toConstruct({
            kind: 'aggregation_expression',
            head: head,
            tail,
          });
        },
        peg$c237 = peg$otherExpectation('InfixedBindingExpression'),
        peg$c238 = function (head, operator, item) {
          return toConstruct({
            kind: 'prefixed_binding_expression',
            operator: operator,
            item: item,
          });
        },
        peg$c239 = function (head, tail) {
          return toConstruct({
            kind: 'infixed_binding_expression',
            head: head,
            tail,
          });
        },
        peg$c240 = peg$otherExpectation('InfixedReductionExpression'),
        peg$c241 = function (head, operator, item) {
          return toConstruct({
            kind: 'prefixed_reduction_expression',
            operator: operator,
            item: item,
          });
        },
        peg$c242 = function (head, tail) {
          return toConstruct({
            kind: 'infixed_reduction_expression',
            head: head,
            tail,
          });
        },
        peg$c243 = peg$otherExpectation('InfixedTransformationExpression'),
        peg$c244 = function (head, operator, item) {
          return toConstruct({
            kind: 'prefixed_transformation_expression',
            operator: operator,
            item: item,
          });
        },
        peg$c245 = function (head, tail) {
          return toConstruct({
            kind: 'infixed_transformation_expression',
            head: head,
            tail,
          });
        },
        peg$c246 = peg$otherExpectation('InfixedRangeExpression'),
        peg$c247 = function (head, operator, item) {
          return toConstruct({ kind: 'prefixed_range_expression', operator: operator, item: item });
        },
        peg$c248 = function (head, tail) {
          return toConstruct({
            kind: 'infixed_range_expression',
            head: head,
            tail,
          });
        },
        peg$c249 = peg$otherExpectation('InfixExpression'),
        peg$c250 = peg$otherExpectation('PrefixExpression'),
        peg$c251 = function (operator) {
          return null;
        },
        peg$c252 = function (operator, operands) {
          return toConstruct({ kind: 'prefix_expression', operator, operands });
        },
        peg$c253 = peg$otherExpectation('PostfixExpression'),
        peg$c254 = function (operand) {
          return null;
        },
        peg$c255 = function (operand) {
          return operand;
        },
        peg$c256 = function (operands) {
          return null;
        },
        peg$c257 = function (operands, operator) {
          const expression = {
            kind: 'postfix_expression',
            operator,
            operands,
          };
          return toConstruct(expression);
        },
        peg$currPos = 0,
        peg$savedPos = 0,
        peg$posDetailsCache = [{ line: 1, column: 1 }],
        peg$maxFailPos = 0,
        peg$maxFailExpected = [],
        peg$silentFails = 0,
        peg$resultsCache = {},
        peg$result;

      if ('startRule' in options) {
        if (!(options.startRule in peg$startRuleFunctions)) {
          throw new Error('Can\'t start parsing from rule "' + options.startRule + '".');
        }

        peg$startRuleFunction = peg$startRuleFunctions[options.startRule];
      }

      function text() {
        return input.substring(peg$savedPos, peg$currPos);
      }

      function location() {
        return peg$computeLocation(peg$savedPos, peg$currPos);
      }

      function expected(description, location) {
        location = location !== void 0 ? location : peg$computeLocation(peg$savedPos, peg$currPos);

        throw peg$buildStructuredError(
          [peg$otherExpectation(description)],
          input.substring(peg$savedPos, peg$currPos),
          location,
        );
      }

      function error(message, location) {
        location = location !== void 0 ? location : peg$computeLocation(peg$savedPos, peg$currPos);

        throw peg$buildSimpleError(message, location);
      }

      function peg$literalExpectation(text, ignoreCase) {
        return { type: 'literal', text: text, ignoreCase: ignoreCase };
      }

      function peg$classExpectation(parts, inverted, ignoreCase) {
        return { type: 'class', parts: parts, inverted: inverted, ignoreCase: ignoreCase };
      }

      function peg$anyExpectation() {
        return { type: 'any' };
      }

      function peg$endExpectation() {
        return { type: 'end' };
      }

      function peg$otherExpectation(description) {
        return { type: 'other', description: description };
      }

      function peg$computePosDetails(pos) {
        var details = peg$posDetailsCache[pos],
          p;

        if (details) {
          return details;
        } else {
          p = pos - 1;
          while (!peg$posDetailsCache[p]) {
            p--;
          }

          details = peg$posDetailsCache[p];
          details = {
            line: details.line,
            column: details.column,
          };

          while (p < pos) {
            if (input.charCodeAt(p) === 10) {
              details.line++;
              details.column = 1;
            } else {
              details.column++;
            }

            p++;
          }

          peg$posDetailsCache[pos] = details;
          return details;
        }
      }

      function peg$computeLocation(startPos, endPos) {
        var startPosDetails = peg$computePosDetails(startPos),
          endPosDetails = peg$computePosDetails(endPos);

        return {
          start: {
            offset: startPos,
            line: startPosDetails.line,
            column: startPosDetails.column,
          },
          end: {
            offset: endPos,
            line: endPosDetails.line,
            column: endPosDetails.column,
          },
        };
      }

      function peg$fail(expected) {
        if (peg$currPos < peg$maxFailPos) {
          return;
        }

        if (peg$currPos > peg$maxFailPos) {
          peg$maxFailPos = peg$currPos;
          peg$maxFailExpected = [];
        }

        peg$maxFailExpected.push(expected);
      }

      function peg$buildSimpleError(message, location) {
        return new peg$SyntaxError(message, null, null, location);
      }

      function peg$buildStructuredError(expected, found, location) {
        return new peg$SyntaxError(
          peg$SyntaxError.buildMessage(expected, found),
          expected,
          found,
          location,
        );
      }

      function peg$parseTop() {
        var s0, s1, s2, s3, s4, s5;

        var key = peg$currPos * 68 + 0,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = [];
        s2 = peg$currPos;
        s3 = peg$parseSpace();
        if (s3 !== peg$FAILED) {
          peg$savedPos = s2;
          s3 = peg$c1();
        }
        s2 = s3;
        if (s2 === peg$FAILED) {
          if (peg$c2.test(input.charAt(peg$currPos))) {
            s2 = input.charAt(peg$currPos);
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c3);
            }
          }
        }
        while (s2 !== peg$FAILED) {
          s1.push(s2);
          s2 = peg$currPos;
          s3 = peg$parseSpace();
          if (s3 !== peg$FAILED) {
            peg$savedPos = s2;
            s3 = peg$c1();
          }
          s2 = s3;
          if (s2 === peg$FAILED) {
            if (peg$c2.test(input.charAt(peg$currPos))) {
              s2 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s2 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c3);
              }
            }
          }
        }
        if (s1 !== peg$FAILED) {
          s2 = peg$parseBlockExpression();
          if (s2 === peg$FAILED) {
            s2 = peg$parseExpression();
            if (s2 === peg$FAILED) {
              s2 = peg$parseContainer();
              if (s2 === peg$FAILED) {
                s2 = peg$parseNode();
              }
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = [];
            s4 = peg$currPos;
            s5 = peg$parseSpace();
            if (s5 !== peg$FAILED) {
              peg$savedPos = s4;
              s5 = peg$c4(s2);
            }
            s4 = s5;
            if (s4 === peg$FAILED) {
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s4 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
            }
            while (s4 !== peg$FAILED) {
              s3.push(s4);
              s4 = peg$currPos;
              s5 = peg$parseSpace();
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c4(s2);
              }
              s4 = s5;
              if (s4 === peg$FAILED) {
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s4 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s4 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
              }
            }
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c5(s2);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c0);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseUnicodeWithoutQuotes() {
        var s0, s1;

        var key = peg$currPos * 68 + 1,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        if (peg$c7.test(input.charAt(peg$currPos))) {
          s0 = input.charAt(peg$currPos);
          peg$currPos++;
        } else {
          s0 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c8);
          }
        }
        if (s0 === peg$FAILED) {
          if (peg$c9.test(input.charAt(peg$currPos))) {
            s0 = input.charAt(peg$currPos);
            peg$currPos++;
          } else {
            s0 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c10);
            }
          }
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c6);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseSpace() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 2,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = [];
        s2 = [];
        if (peg$c12.test(input.charAt(peg$currPos))) {
          s3 = input.charAt(peg$currPos);
          peg$currPos++;
        } else {
          s3 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c13);
          }
        }
        if (s3 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 32) {
            s3 = peg$c14;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c15);
            }
          }
          if (s3 === peg$FAILED) {
            s3 = peg$currPos;
            if (peg$c2.test(input.charAt(peg$currPos))) {
              s4 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c3);
              }
            }
            if (s4 !== peg$FAILED) {
              peg$savedPos = s3;
              s4 = peg$c16(s4);
            }
            s3 = s4;
          }
        }
        if (s3 !== peg$FAILED) {
          while (s3 !== peg$FAILED) {
            s2.push(s3);
            if (peg$c12.test(input.charAt(peg$currPos))) {
              s3 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s3 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c13);
              }
            }
            if (s3 === peg$FAILED) {
              if (input.charCodeAt(peg$currPos) === 32) {
                s3 = peg$c14;
                peg$currPos++;
              } else {
                s3 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c15);
                }
              }
              if (s3 === peg$FAILED) {
                s3 = peg$currPos;
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s4 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s4 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
                if (s4 !== peg$FAILED) {
                  peg$savedPos = s3;
                  s4 = peg$c16(s4);
                }
                s3 = s4;
              }
            }
          }
        } else {
          s2 = peg$FAILED;
        }
        if (s2 !== peg$FAILED) {
          while (s2 !== peg$FAILED) {
            s1.push(s2);
            s2 = [];
            if (peg$c12.test(input.charAt(peg$currPos))) {
              s3 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s3 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c13);
              }
            }
            if (s3 === peg$FAILED) {
              if (input.charCodeAt(peg$currPos) === 32) {
                s3 = peg$c14;
                peg$currPos++;
              } else {
                s3 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c15);
                }
              }
              if (s3 === peg$FAILED) {
                s3 = peg$currPos;
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s4 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s4 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
                if (s4 !== peg$FAILED) {
                  peg$savedPos = s3;
                  s4 = peg$c16(s4);
                }
                s3 = s4;
              }
            }
            if (s3 !== peg$FAILED) {
              while (s3 !== peg$FAILED) {
                s2.push(s3);
                if (peg$c12.test(input.charAt(peg$currPos))) {
                  s3 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s3 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c13);
                  }
                }
                if (s3 === peg$FAILED) {
                  if (input.charCodeAt(peg$currPos) === 32) {
                    s3 = peg$c14;
                    peg$currPos++;
                  } else {
                    s3 = peg$FAILED;
                    if (peg$silentFails === 0) {
                      peg$fail(peg$c15);
                    }
                  }
                  if (s3 === peg$FAILED) {
                    s3 = peg$currPos;
                    if (peg$c2.test(input.charAt(peg$currPos))) {
                      s4 = input.charAt(peg$currPos);
                      peg$currPos++;
                    } else {
                      s4 = peg$FAILED;
                      if (peg$silentFails === 0) {
                        peg$fail(peg$c3);
                      }
                    }
                    if (s4 !== peg$FAILED) {
                      peg$savedPos = s3;
                      s4 = peg$c16(s4);
                    }
                    s3 = s4;
                  }
                }
              }
            } else {
              s2 = peg$FAILED;
            }
          }
        } else {
          s1 = peg$FAILED;
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c17(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c11);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseNode() {
        var s0, s1;

        var key = peg$currPos * 68 + 3,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$parsePhraseNode();
        if (s0 === peg$FAILED) {
          s0 = peg$parseEmbedmentNode();
          if (s0 === peg$FAILED) {
            s0 = peg$parseStringNode();
            if (s0 === peg$FAILED) {
              s0 = peg$parseNumberNode();
              if (s0 === peg$FAILED) {
                s0 = peg$parseAnchorNode();
              }
            }
          }
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c18);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseAnchorNode() {
        var s0, s1, s2, s3, s4, s5;

        var key = peg$currPos * 68 + 4,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        s2 = [];
        s3 = peg$currPos;
        s4 = [];
        if (peg$c20.test(input.charAt(peg$currPos))) {
          s5 = input.charAt(peg$currPos);
          peg$currPos++;
        } else {
          s5 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c21);
          }
        }
        if (s5 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s5 = peg$c22;
            peg$currPos++;
          } else {
            s5 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
        }
        if (s5 !== peg$FAILED) {
          while (s5 !== peg$FAILED) {
            s4.push(s5);
            if (peg$c20.test(input.charAt(peg$currPos))) {
              s5 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s5 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c21);
              }
            }
            if (s5 === peg$FAILED) {
              if (input.charCodeAt(peg$currPos) === 95) {
                s5 = peg$c22;
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c23);
                }
              }
            }
          }
        } else {
          s4 = peg$FAILED;
        }
        if (s4 !== peg$FAILED) {
          peg$savedPos = s3;
          s4 = peg$c24(s4);
        }
        s3 = s4;
        if (s3 !== peg$FAILED) {
          while (s3 !== peg$FAILED) {
            s2.push(s3);
            s3 = peg$currPos;
            s4 = [];
            if (peg$c20.test(input.charAt(peg$currPos))) {
              s5 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s5 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c21);
              }
            }
            if (s5 === peg$FAILED) {
              if (input.charCodeAt(peg$currPos) === 95) {
                s5 = peg$c22;
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c23);
                }
              }
            }
            if (s5 !== peg$FAILED) {
              while (s5 !== peg$FAILED) {
                s4.push(s5);
                if (peg$c20.test(input.charAt(peg$currPos))) {
                  s5 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s5 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c21);
                  }
                }
                if (s5 === peg$FAILED) {
                  if (input.charCodeAt(peg$currPos) === 95) {
                    s5 = peg$c22;
                    peg$currPos++;
                  } else {
                    s5 = peg$FAILED;
                    if (peg$silentFails === 0) {
                      peg$fail(peg$c23);
                    }
                  }
                }
              }
            } else {
              s4 = peg$FAILED;
            }
            if (s4 !== peg$FAILED) {
              peg$savedPos = s3;
              s4 = peg$c24(s4);
            }
            s3 = s4;
          }
        } else {
          s2 = peg$FAILED;
        }
        if (s2 !== peg$FAILED) {
          peg$savedPos = s1;
          s2 = peg$c25(s2);
        }
        s1 = s2;
        if (s1 === peg$FAILED) {
          s1 = peg$currPos;
          s2 = [];
          if (peg$c26.test(input.charAt(peg$currPos))) {
            s3 = input.charAt(peg$currPos);
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c27);
            }
          }
          if (s3 !== peg$FAILED) {
            while (s3 !== peg$FAILED) {
              s2.push(s3);
              if (peg$c26.test(input.charAt(peg$currPos))) {
                s3 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s3 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c27);
                }
              }
            }
          } else {
            s2 = peg$FAILED;
          }
          if (s2 !== peg$FAILED) {
            s3 = [];
            if (peg$c20.test(input.charAt(peg$currPos))) {
              s4 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c21);
              }
            }
            while (s4 !== peg$FAILED) {
              s3.push(s4);
              if (peg$c20.test(input.charAt(peg$currPos))) {
                s4 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c21);
                }
              }
            }
            if (s3 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c28(s2, s3);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c29(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c19);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseEmbedmentNode() {
        var s0, s1, s2, s3, s4, s5;

        var key = peg$currPos * 68 + 5,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (peg$c31.test(input.charAt(peg$currPos))) {
          s2 = input.charAt(peg$currPos);
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c32);
          }
        }
        if (s2 !== peg$FAILED) {
          s3 = [];
          s4 = peg$parseUnicodeWithoutQuotes();
          if (s4 === peg$FAILED) {
            if (peg$c33.test(input.charAt(peg$currPos))) {
              s4 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c34);
              }
            }
            if (s4 === peg$FAILED) {
              s4 = peg$currPos;
              s5 = peg$parseSpace();
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c1();
              }
              s4 = s5;
              if (s4 === peg$FAILED) {
                if (peg$c35.test(input.charAt(peg$currPos))) {
                  s4 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s4 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c36);
                  }
                }
                if (s4 === peg$FAILED) {
                  if (peg$c37.test(input.charAt(peg$currPos))) {
                    s4 = input.charAt(peg$currPos);
                    peg$currPos++;
                  } else {
                    s4 = peg$FAILED;
                    if (peg$silentFails === 0) {
                      peg$fail(peg$c38);
                    }
                  }
                  if (s4 === peg$FAILED) {
                    if (peg$c2.test(input.charAt(peg$currPos))) {
                      s4 = input.charAt(peg$currPos);
                      peg$currPos++;
                    } else {
                      s4 = peg$FAILED;
                      if (peg$silentFails === 0) {
                        peg$fail(peg$c3);
                      }
                    }
                  }
                }
              }
            }
          }
          while (s4 !== peg$FAILED) {
            s3.push(s4);
            s4 = peg$parseUnicodeWithoutQuotes();
            if (s4 === peg$FAILED) {
              if (peg$c33.test(input.charAt(peg$currPos))) {
                s4 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c34);
                }
              }
              if (s4 === peg$FAILED) {
                s4 = peg$currPos;
                s5 = peg$parseSpace();
                if (s5 !== peg$FAILED) {
                  peg$savedPos = s4;
                  s5 = peg$c1();
                }
                s4 = s5;
                if (s4 === peg$FAILED) {
                  if (peg$c35.test(input.charAt(peg$currPos))) {
                    s4 = input.charAt(peg$currPos);
                    peg$currPos++;
                  } else {
                    s4 = peg$FAILED;
                    if (peg$silentFails === 0) {
                      peg$fail(peg$c36);
                    }
                  }
                  if (s4 === peg$FAILED) {
                    if (peg$c37.test(input.charAt(peg$currPos))) {
                      s4 = input.charAt(peg$currPos);
                      peg$currPos++;
                    } else {
                      s4 = peg$FAILED;
                      if (peg$silentFails === 0) {
                        peg$fail(peg$c38);
                      }
                    }
                    if (s4 === peg$FAILED) {
                      if (peg$c2.test(input.charAt(peg$currPos))) {
                        s4 = input.charAt(peg$currPos);
                        peg$currPos++;
                      } else {
                        s4 = peg$FAILED;
                        if (peg$silentFails === 0) {
                          peg$fail(peg$c3);
                        }
                      }
                    }
                  }
                }
              }
            }
          }
          if (s3 !== peg$FAILED) {
            if (peg$c31.test(input.charAt(peg$currPos))) {
              s4 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c32);
              }
            }
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c39(s3);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c40(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c30);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseNumberNode() {
        var s0, s1, s2;

        var key = peg$currPos * 68 + 6,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = [];
        if (peg$c42.test(input.charAt(peg$currPos))) {
          s2 = input.charAt(peg$currPos);
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c43);
          }
        }
        if (s2 !== peg$FAILED) {
          while (s2 !== peg$FAILED) {
            s1.push(s2);
            if (peg$c42.test(input.charAt(peg$currPos))) {
              s2 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s2 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c43);
              }
            }
          }
        } else {
          s1 = peg$FAILED;
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c44(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c41);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parsePhraseNode() {
        var s0, s1, s2, s3, s4, s5, s6;

        var key = peg$currPos * 68 + 7,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        s2 = peg$parseAnchorNode();
        if (s2 === peg$FAILED) {
          s2 = peg$parseNumberNode();
        }
        if (s2 !== peg$FAILED) {
          s3 = [];
          s4 = peg$currPos;
          s5 = [];
          if (input.charCodeAt(peg$currPos) === 32) {
            s6 = peg$c14;
            peg$currPos++;
          } else {
            s6 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c15);
            }
          }
          if (s6 === peg$FAILED) {
            if (peg$c12.test(input.charAt(peg$currPos))) {
              s6 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s6 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c13);
              }
            }
          }
          if (s6 !== peg$FAILED) {
            while (s6 !== peg$FAILED) {
              s5.push(s6);
              if (input.charCodeAt(peg$currPos) === 32) {
                s6 = peg$c14;
                peg$currPos++;
              } else {
                s6 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c15);
                }
              }
              if (s6 === peg$FAILED) {
                if (peg$c12.test(input.charAt(peg$currPos))) {
                  s6 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s6 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c13);
                  }
                }
              }
            }
          } else {
            s5 = peg$FAILED;
          }
          if (s5 !== peg$FAILED) {
            s6 = peg$parseAnchorNode();
            if (s6 === peg$FAILED) {
              s6 = peg$parseNumberNode();
            }
            if (s6 !== peg$FAILED) {
              peg$savedPos = s4;
              s5 = peg$c46(s2, s6);
              s4 = s5;
            } else {
              peg$currPos = s4;
              s4 = peg$FAILED;
            }
          } else {
            peg$currPos = s4;
            s4 = peg$FAILED;
          }
          if (s4 !== peg$FAILED) {
            while (s4 !== peg$FAILED) {
              s3.push(s4);
              s4 = peg$currPos;
              s5 = [];
              if (input.charCodeAt(peg$currPos) === 32) {
                s6 = peg$c14;
                peg$currPos++;
              } else {
                s6 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c15);
                }
              }
              if (s6 === peg$FAILED) {
                if (peg$c12.test(input.charAt(peg$currPos))) {
                  s6 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s6 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c13);
                  }
                }
              }
              if (s6 !== peg$FAILED) {
                while (s6 !== peg$FAILED) {
                  s5.push(s6);
                  if (input.charCodeAt(peg$currPos) === 32) {
                    s6 = peg$c14;
                    peg$currPos++;
                  } else {
                    s6 = peg$FAILED;
                    if (peg$silentFails === 0) {
                      peg$fail(peg$c15);
                    }
                  }
                  if (s6 === peg$FAILED) {
                    if (peg$c12.test(input.charAt(peg$currPos))) {
                      s6 = input.charAt(peg$currPos);
                      peg$currPos++;
                    } else {
                      s6 = peg$FAILED;
                      if (peg$silentFails === 0) {
                        peg$fail(peg$c13);
                      }
                    }
                  }
                }
              } else {
                s5 = peg$FAILED;
              }
              if (s5 !== peg$FAILED) {
                s6 = peg$parseAnchorNode();
                if (s6 === peg$FAILED) {
                  s6 = peg$parseNumberNode();
                }
                if (s6 !== peg$FAILED) {
                  peg$savedPos = s4;
                  s5 = peg$c46(s2, s6);
                  s4 = s5;
                } else {
                  peg$currPos = s4;
                  s4 = peg$FAILED;
                }
              } else {
                peg$currPos = s4;
                s4 = peg$FAILED;
              }
            }
          } else {
            s3 = peg$FAILED;
          }
          if (s3 !== peg$FAILED) {
            peg$savedPos = s1;
            s2 = peg$c47(s2, s3);
            s1 = s2;
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c48(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c45);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseScalar() {
        var s0, s1;

        var key = peg$currPos * 68 + 8,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$parsePhraseNode();
        if (s0 === peg$FAILED) {
          s0 = peg$parseEmbedmentNode();
          if (s0 === peg$FAILED) {
            s0 = peg$parseStringNode();
            if (s0 === peg$FAILED) {
              s0 = peg$parseNumberNode();
              if (s0 === peg$FAILED) {
                s0 = peg$parseAnchorNode();
              }
            }
          }
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c49);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseStringNode() {
        var s0, s1, s2, s3, s4, s5, s6;

        var key = peg$currPos * 68 + 9,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (peg$c37.test(input.charAt(peg$currPos))) {
          s2 = input.charAt(peg$currPos);
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c38);
          }
        }
        if (s2 !== peg$FAILED) {
          s3 = [];
          s4 = peg$parseUnicodeWithoutQuotes();
          if (s4 === peg$FAILED) {
            if (peg$c2.test(input.charAt(peg$currPos))) {
              s4 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c3);
              }
            }
            if (s4 === peg$FAILED) {
              if (peg$c35.test(input.charAt(peg$currPos))) {
                s4 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c36);
                }
              }
            }
          }
          while (s4 !== peg$FAILED) {
            s3.push(s4);
            s4 = peg$parseUnicodeWithoutQuotes();
            if (s4 === peg$FAILED) {
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s4 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
              if (s4 === peg$FAILED) {
                if (peg$c35.test(input.charAt(peg$currPos))) {
                  s4 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s4 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c36);
                  }
                }
              }
            }
          }
          if (s3 !== peg$FAILED) {
            if (peg$c37.test(input.charAt(peg$currPos))) {
              s4 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c38);
              }
            }
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c39(s3);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          s1 = peg$currPos;
          if (peg$c35.test(input.charAt(peg$currPos))) {
            s2 = input.charAt(peg$currPos);
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c36);
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = [];
            s4 = peg$currPos;
            if (input.charCodeAt(peg$currPos) === 92) {
              s5 = peg$c51;
              peg$currPos++;
            } else {
              s5 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c52);
              }
            }
            if (s5 !== peg$FAILED) {
              if (peg$c35.test(input.charAt(peg$currPos))) {
                s6 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s6 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c36);
                }
              }
              if (s6 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c53();
                s4 = s5;
              } else {
                peg$currPos = s4;
                s4 = peg$FAILED;
              }
            } else {
              peg$currPos = s4;
              s4 = peg$FAILED;
            }
            if (s4 === peg$FAILED) {
              s4 = peg$parseUnicodeWithoutQuotes();
              if (s4 === peg$FAILED) {
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s4 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s4 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
                if (s4 === peg$FAILED) {
                  if (peg$c37.test(input.charAt(peg$currPos))) {
                    s4 = input.charAt(peg$currPos);
                    peg$currPos++;
                  } else {
                    s4 = peg$FAILED;
                    if (peg$silentFails === 0) {
                      peg$fail(peg$c38);
                    }
                  }
                }
              }
            }
            while (s4 !== peg$FAILED) {
              s3.push(s4);
              s4 = peg$currPos;
              if (input.charCodeAt(peg$currPos) === 92) {
                s5 = peg$c51;
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c52);
                }
              }
              if (s5 !== peg$FAILED) {
                if (peg$c35.test(input.charAt(peg$currPos))) {
                  s6 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s6 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c36);
                  }
                }
                if (s6 !== peg$FAILED) {
                  peg$savedPos = s4;
                  s5 = peg$c53();
                  s4 = s5;
                } else {
                  peg$currPos = s4;
                  s4 = peg$FAILED;
                }
              } else {
                peg$currPos = s4;
                s4 = peg$FAILED;
              }
              if (s4 === peg$FAILED) {
                s4 = peg$parseUnicodeWithoutQuotes();
                if (s4 === peg$FAILED) {
                  if (peg$c2.test(input.charAt(peg$currPos))) {
                    s4 = input.charAt(peg$currPos);
                    peg$currPos++;
                  } else {
                    s4 = peg$FAILED;
                    if (peg$silentFails === 0) {
                      peg$fail(peg$c3);
                    }
                  }
                  if (s4 === peg$FAILED) {
                    if (peg$c37.test(input.charAt(peg$currPos))) {
                      s4 = input.charAt(peg$currPos);
                      peg$currPos++;
                    } else {
                      s4 = peg$FAILED;
                      if (peg$silentFails === 0) {
                        peg$fail(peg$c38);
                      }
                    }
                  }
                }
              }
            }
            if (s3 !== peg$FAILED) {
              if (peg$c35.test(input.charAt(peg$currPos))) {
                s4 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c36);
                }
              }
              if (s4 !== peg$FAILED) {
                peg$savedPos = s1;
                s2 = peg$c39(s3);
                s1 = s2;
              } else {
                peg$currPos = s1;
                s1 = peg$FAILED;
              }
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c54(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c50);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseOperator() {
        var s0, s1;

        var key = peg$currPos * 68 + 10,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$parseSpreadOperator();
        if (s0 === peg$FAILED) {
          s0 = peg$parseRangeOperator();
          if (s0 === peg$FAILED) {
            s0 = peg$parseDescentOperator();
            if (s0 === peg$FAILED) {
              s0 = peg$parseTransformationOperator();
              if (s0 === peg$FAILED) {
                s0 = peg$parseDirectionOperator();
                if (s0 === peg$FAILED) {
                  s0 = peg$parseAggregationOperator();
                  if (s0 === peg$FAILED) {
                    s0 = peg$parseAscentOperator();
                    if (s0 === peg$FAILED) {
                      s0 = peg$parseBindingOperator();
                      if (s0 === peg$FAILED) {
                        s0 = peg$parseBranchOperator();
                        if (s0 === peg$FAILED) {
                          s0 = peg$parseChannelOperator();
                          if (s0 === peg$FAILED) {
                            s0 = peg$parseEvaluationOperator();
                            if (s0 === peg$FAILED) {
                              s0 = peg$parseInvocationOperator();
                              if (s0 === peg$FAILED) {
                                s0 = peg$parsePerformanceOperator();
                                if (s0 === peg$FAILED) {
                                  s0 = peg$parsePerspectiveOperator();
                                  if (s0 === peg$FAILED) {
                                    s0 = peg$parseReductionOperator();
                                    if (s0 === peg$FAILED) {
                                      s0 = peg$parseReferenceOperator();
                                      if (s0 === peg$FAILED) {
                                        s0 = peg$parseValueOperator();
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c55);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseAggregationOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 11,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 43) {
          s2 = peg$c57;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c58);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 43) {
            s1 = peg$c57;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c58);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c60(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c56);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseAscentOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 12,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 94) {
          s2 = peg$c62;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c63);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 94) {
            s1 = peg$c62;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c63);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c64(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c61);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseBindingOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 13,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 58) {
          s2 = peg$c66;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c67);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 58) {
            s1 = peg$c66;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c67);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c68(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c65);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseBranchOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 14,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 124) {
          s2 = peg$c70;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c71);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 124) {
            s1 = peg$c70;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c71);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c72(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c69);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseChannelOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 15,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 35) {
          s2 = peg$c74;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c75);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 35) {
            s1 = peg$c74;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c75);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c76(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c73);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseDescentOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 16,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 46) {
          s2 = peg$c78;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c79);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 46) {
            s1 = peg$c78;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c79);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c80(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c77);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseDirectionOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 17,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.substr(peg$currPos, 2) === peg$c82) {
          s2 = peg$c82;
          peg$currPos += 2;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c83);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.substr(peg$currPos, 2) === peg$c82) {
            s1 = peg$c82;
            peg$currPos += 2;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c83);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c84(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c81);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseEvaluationOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 18,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 63) {
          s2 = peg$c86;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c87);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 63) {
            s1 = peg$c86;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c87);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c88(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c85);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseInvocationOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 19,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 126) {
          s2 = peg$c90;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c91);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 126) {
            s1 = peg$c90;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c91);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c92(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c89);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parsePerformanceOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 20,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 33) {
          s2 = peg$c94;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c95);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 33) {
            s1 = peg$c94;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c95);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c96(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c93);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parsePerspectiveOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 21,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 64) {
          s2 = peg$c98;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c99);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 64) {
            s1 = peg$c98;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c99);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c100(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c97);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseRangeOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 22,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.substr(peg$currPos, 2) === peg$c102) {
          s2 = peg$c102;
          peg$currPos += 2;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c103);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.substr(peg$currPos, 2) === peg$c102) {
            s1 = peg$c102;
            peg$currPos += 2;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c103);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c104(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c101);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseReductionOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 23,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 45) {
          s2 = peg$c106;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c107);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 45) {
            s1 = peg$c106;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c107);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c108(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c105);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseReferenceOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 24,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 38) {
          s2 = peg$c110;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c111);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 38) {
            s1 = peg$c110;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c111);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c112(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c109);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseSpreadOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 25,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.substr(peg$currPos, 3) === peg$c114) {
          s2 = peg$c114;
          peg$currPos += 3;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c115);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.substr(peg$currPos, 3) === peg$c114) {
            s1 = peg$c114;
            peg$currPos += 3;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c115);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c116(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c113);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseTransformationOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 26,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.substr(peg$currPos, 2) === peg$c118) {
          s2 = peg$c118;
          peg$currPos += 2;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c119);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.substr(peg$currPos, 2) === peg$c118) {
            s1 = peg$c118;
            peg$currPos += 2;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c119);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c120(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c117);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseValueOperator() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 27,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 42) {
          s2 = peg$c122;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c123);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 42) {
            s1 = peg$c122;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c123);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c124(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c121);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseBlockDelimiter() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 28,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 59) {
          s2 = peg$c126;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c127);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 59) {
            s1 = peg$c126;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c127);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c128(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c125);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseCommonDelimiter() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 29,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 44) {
          s2 = peg$c130;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c131);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 44) {
            s1 = peg$c130;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c131);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c132(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c129);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseNodeDelimiter() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 30,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 32) {
          s2 = peg$c14;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c15);
          }
        }
        if (s2 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s3 = peg$c22;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseAnchorNode();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c59(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 32) {
            s1 = peg$c14;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c15);
            }
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c134(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c133);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseContainer() {
        var s0, s1;

        var key = peg$currPos * 68 + 31,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$parseConcept();
        if (s0 === peg$FAILED) {
          s0 = peg$parseLocation();
          if (s0 === peg$FAILED) {
            s0 = peg$parseDomain();
            if (s0 === peg$FAILED) {
              s0 = peg$parseEssence();
            }
          }
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c135);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseDomainOpen() {
        var s0, s1, s2, s3, s4, s5;

        var key = peg$currPos * 68 + 32,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 123) {
          s1 = peg$c137;
          peg$currPos++;
        } else {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c138);
          }
        }
        if (s1 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s2 = peg$c22;
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = peg$currPos;
            s4 = peg$parseAnchorNode();
            if (s4 === peg$FAILED) {
              s4 = peg$parseConcept();
              if (s4 === peg$FAILED) {
                s4 = peg$parseLocation();
                if (s4 === peg$FAILED) {
                  s4 = peg$parseDomain();
                  if (s4 === peg$FAILED) {
                    s4 = peg$parseEssence();
                  }
                }
              }
            }
            if (s4 !== peg$FAILED) {
              s5 = peg$parseConcept();
              if (s5 === peg$FAILED) {
                s5 = peg$parseLocation();
                if (s5 === peg$FAILED) {
                  s5 = peg$parseDomain();
                  if (s5 === peg$FAILED) {
                    s5 = peg$parseEssence();
                  }
                }
              }
              if (s5 === peg$FAILED) {
                s5 = null;
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s3;
                s4 = peg$c139(s1, s4, s5);
                s3 = s4;
              } else {
                peg$currPos = s3;
                s3 = peg$FAILED;
              }
            } else {
              peg$currPos = s3;
              s3 = peg$FAILED;
            }
            if (s3 !== peg$FAILED) {
              s4 = peg$currPos;
              s5 = peg$parseSpace();
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c140(s1, s3);
              }
              s4 = s5;
              if (s4 !== peg$FAILED) {
                peg$savedPos = s0;
                s1 = peg$c141(s1, s3);
                s0 = s1;
              } else {
                peg$currPos = s0;
                s0 = peg$FAILED;
              }
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        if (s0 === peg$FAILED) {
          s0 = peg$currPos;
          if (input.charCodeAt(peg$currPos) === 123) {
            s1 = peg$c137;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c138);
            }
          }
          if (s1 !== peg$FAILED) {
            peg$savedPos = s0;
            s1 = peg$c142(s1);
          }
          s0 = s1;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c136);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseDomainClose() {
        var s0, s1, s2, s3;

        var key = peg$currPos * 68 + 33,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 125) {
          s1 = peg$c144;
          peg$currPos++;
        } else {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c145);
          }
        }
        if (s1 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s2 = peg$c22;
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = peg$parseAnchorNode();
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c146(s1, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        if (s0 === peg$FAILED) {
          s0 = peg$currPos;
          s1 = peg$parseAnchorNode();
          if (s1 !== peg$FAILED) {
            if (input.charCodeAt(peg$currPos) === 95) {
              s2 = peg$c22;
              peg$currPos++;
            } else {
              s2 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c23);
              }
            }
            if (s2 !== peg$FAILED) {
              if (input.charCodeAt(peg$currPos) === 125) {
                s3 = peg$c144;
                peg$currPos++;
              } else {
                s3 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c145);
                }
              }
              if (s3 !== peg$FAILED) {
                peg$savedPos = s0;
                s1 = peg$c147(s1, s3);
                s0 = s1;
              } else {
                peg$currPos = s0;
                s0 = peg$FAILED;
              }
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        }
        if (s0 === peg$FAILED) {
          s0 = peg$currPos;
          if (input.charCodeAt(peg$currPos) === 125) {
            s1 = peg$c144;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c145);
            }
          }
          if (s1 !== peg$FAILED) {
            peg$savedPos = s0;
            s1 = peg$c148(s1);
          }
          s0 = s1;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c143);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseDomainBody() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 34,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = [];
        if (input.charCodeAt(peg$currPos) === 32) {
          s2 = peg$c14;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c15);
          }
        }
        if (s2 === peg$FAILED) {
          if (peg$c2.test(input.charAt(peg$currPos))) {
            s2 = input.charAt(peg$currPos);
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c3);
            }
          }
        }
        while (s2 !== peg$FAILED) {
          s1.push(s2);
          if (input.charCodeAt(peg$currPos) === 32) {
            s2 = peg$c14;
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c15);
            }
          }
          if (s2 === peg$FAILED) {
            if (peg$c2.test(input.charAt(peg$currPos))) {
              s2 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s2 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c3);
              }
            }
          }
        }
        if (s1 !== peg$FAILED) {
          s2 = peg$parseBlockExpression();
          if (s2 === peg$FAILED) {
            s2 = peg$parseChannelOperator();
            if (s2 === peg$FAILED) {
              s2 = peg$parseExpression();
              if (s2 === peg$FAILED) {
                s2 = peg$parseContainer();
                if (s2 === peg$FAILED) {
                  s2 = peg$parseNode();
                  if (s2 === peg$FAILED) {
                    s2 = [];
                    if (input.charCodeAt(peg$currPos) === 32) {
                      s3 = peg$c14;
                      peg$currPos++;
                    } else {
                      s3 = peg$FAILED;
                      if (peg$silentFails === 0) {
                        peg$fail(peg$c15);
                      }
                    }
                    if (s3 === peg$FAILED) {
                      if (peg$c2.test(input.charAt(peg$currPos))) {
                        s3 = input.charAt(peg$currPos);
                        peg$currPos++;
                      } else {
                        s3 = peg$FAILED;
                        if (peg$silentFails === 0) {
                          peg$fail(peg$c3);
                        }
                      }
                    }
                    while (s3 !== peg$FAILED) {
                      s2.push(s3);
                      if (input.charCodeAt(peg$currPos) === 32) {
                        s3 = peg$c14;
                        peg$currPos++;
                      } else {
                        s3 = peg$FAILED;
                        if (peg$silentFails === 0) {
                          peg$fail(peg$c15);
                        }
                      }
                      if (s3 === peg$FAILED) {
                        if (peg$c2.test(input.charAt(peg$currPos))) {
                          s3 = input.charAt(peg$currPos);
                          peg$currPos++;
                        } else {
                          s3 = peg$FAILED;
                          if (peg$silentFails === 0) {
                            peg$fail(peg$c3);
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = [];
            if (input.charCodeAt(peg$currPos) === 32) {
              s4 = peg$c14;
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c15);
              }
            }
            if (s4 === peg$FAILED) {
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s4 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
            }
            while (s4 !== peg$FAILED) {
              s3.push(s4);
              if (input.charCodeAt(peg$currPos) === 32) {
                s4 = peg$c14;
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c15);
                }
              }
              if (s4 === peg$FAILED) {
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s4 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s4 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
              }
            }
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c150(s1, s2, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c149);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseDomain() {
        var s0, s1, s2, s3, s4, s5;

        var key = peg$currPos * 68 + 35,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        s2 = peg$parseDomainOpen();
        if (s2 !== peg$FAILED) {
          s3 = [];
          if (input.charCodeAt(peg$currPos) === 32) {
            s4 = peg$c14;
            peg$currPos++;
          } else {
            s4 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c15);
            }
          }
          if (s4 === peg$FAILED) {
            s4 = peg$currPos;
            if (peg$c12.test(input.charAt(peg$currPos))) {
              s5 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s5 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c13);
              }
            }
            if (s5 !== peg$FAILED) {
              peg$savedPos = s4;
              s5 = peg$c152(s2);
            }
            s4 = s5;
          }
          if (s4 === peg$FAILED) {
            s4 = peg$currPos;
            if (input.charCodeAt(peg$currPos) === 95) {
              s5 = peg$c22;
              peg$currPos++;
            } else {
              s5 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c23);
              }
            }
            if (s5 !== peg$FAILED) {
              peg$savedPos = s4;
              s5 = peg$c153(s2, s5);
            }
            s4 = s5;
            if (s4 === peg$FAILED) {
              s4 = peg$currPos;
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s5 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c152(s2);
              }
              s4 = s5;
            }
          }
          while (s4 !== peg$FAILED) {
            s3.push(s4);
            if (input.charCodeAt(peg$currPos) === 32) {
              s4 = peg$c14;
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c15);
              }
            }
            if (s4 === peg$FAILED) {
              s4 = peg$currPos;
              if (peg$c12.test(input.charAt(peg$currPos))) {
                s5 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c13);
                }
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c152(s2);
              }
              s4 = s5;
            }
            if (s4 === peg$FAILED) {
              s4 = peg$currPos;
              if (input.charCodeAt(peg$currPos) === 95) {
                s5 = peg$c22;
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c23);
                }
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c153(s2, s5);
              }
              s4 = s5;
              if (s4 === peg$FAILED) {
                s4 = peg$currPos;
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s5 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s5 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
                if (s5 !== peg$FAILED) {
                  peg$savedPos = s4;
                  s5 = peg$c152(s2);
                }
                s4 = s5;
              }
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseDomainClose();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c154(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          s1 = peg$currPos;
          s2 = peg$parseDomainOpen();
          if (s2 !== peg$FAILED) {
            s3 = peg$parseDomainBody();
            if (s3 !== peg$FAILED) {
              s4 = peg$parseDomainClose();
              if (s4 !== peg$FAILED) {
                peg$savedPos = s1;
                s2 = peg$c155(s2, s3, s4);
                s1 = s2;
              } else {
                peg$currPos = s1;
                s1 = peg$FAILED;
              }
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c156(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c151);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseEssenceOpen() {
        var s0, s1, s2, s3, s4, s5;

        var key = peg$currPos * 68 + 36,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 91) {
          s1 = peg$c158;
          peg$currPos++;
        } else {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c159);
          }
        }
        if (s1 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s2 = peg$c22;
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = peg$currPos;
            s4 = peg$parseAnchorNode();
            if (s4 === peg$FAILED) {
              s4 = peg$parseConcept();
              if (s4 === peg$FAILED) {
                s4 = peg$parseLocation();
                if (s4 === peg$FAILED) {
                  s4 = peg$parseDomain();
                  if (s4 === peg$FAILED) {
                    s4 = peg$parseEssence();
                  }
                }
              }
            }
            if (s4 !== peg$FAILED) {
              s5 = peg$parseConcept();
              if (s5 === peg$FAILED) {
                s5 = peg$parseLocation();
                if (s5 === peg$FAILED) {
                  s5 = peg$parseDomain();
                  if (s5 === peg$FAILED) {
                    s5 = peg$parseEssence();
                  }
                }
              }
              if (s5 === peg$FAILED) {
                s5 = null;
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s3;
                s4 = peg$c139(s1, s4, s5);
                s3 = s4;
              } else {
                peg$currPos = s3;
                s3 = peg$FAILED;
              }
            } else {
              peg$currPos = s3;
              s3 = peg$FAILED;
            }
            if (s3 !== peg$FAILED) {
              s4 = peg$currPos;
              s5 = peg$parseSpace();
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c140(s1, s3);
              }
              s4 = s5;
              if (s4 !== peg$FAILED) {
                peg$savedPos = s0;
                s1 = peg$c160(s1, s3);
                s0 = s1;
              } else {
                peg$currPos = s0;
                s0 = peg$FAILED;
              }
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        if (s0 === peg$FAILED) {
          s0 = peg$currPos;
          if (input.charCodeAt(peg$currPos) === 91) {
            s1 = peg$c158;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c159);
            }
          }
          if (s1 !== peg$FAILED) {
            peg$savedPos = s0;
            s1 = peg$c161(s1);
          }
          s0 = s1;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c157);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseEssenceClose() {
        var s0, s1, s2, s3;

        var key = peg$currPos * 68 + 37,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 93) {
          s1 = peg$c163;
          peg$currPos++;
        } else {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c164);
          }
        }
        if (s1 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s2 = peg$c22;
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = peg$parseAnchorNode();
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c165(s1, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        if (s0 === peg$FAILED) {
          s0 = peg$currPos;
          s1 = peg$parseAnchorNode();
          if (s1 !== peg$FAILED) {
            if (input.charCodeAt(peg$currPos) === 95) {
              s2 = peg$c22;
              peg$currPos++;
            } else {
              s2 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c23);
              }
            }
            if (s2 !== peg$FAILED) {
              if (input.charCodeAt(peg$currPos) === 93) {
                s3 = peg$c163;
                peg$currPos++;
              } else {
                s3 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c164);
                }
              }
              if (s3 !== peg$FAILED) {
                peg$savedPos = s0;
                s1 = peg$c166(s1, s3);
                s0 = s1;
              } else {
                peg$currPos = s0;
                s0 = peg$FAILED;
              }
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        }
        if (s0 === peg$FAILED) {
          s0 = peg$currPos;
          if (input.charCodeAt(peg$currPos) === 93) {
            s1 = peg$c163;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c164);
            }
          }
          if (s1 !== peg$FAILED) {
            peg$savedPos = s0;
            s1 = peg$c167(s1);
          }
          s0 = s1;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c162);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseEssenceBody() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 38,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = [];
        if (input.charCodeAt(peg$currPos) === 32) {
          s2 = peg$c14;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c15);
          }
        }
        if (s2 === peg$FAILED) {
          if (peg$c2.test(input.charAt(peg$currPos))) {
            s2 = input.charAt(peg$currPos);
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c3);
            }
          }
        }
        while (s2 !== peg$FAILED) {
          s1.push(s2);
          if (input.charCodeAt(peg$currPos) === 32) {
            s2 = peg$c14;
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c15);
            }
          }
          if (s2 === peg$FAILED) {
            if (peg$c2.test(input.charAt(peg$currPos))) {
              s2 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s2 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c3);
              }
            }
          }
        }
        if (s1 !== peg$FAILED) {
          s2 = peg$parseBlockExpression();
          if (s2 === peg$FAILED) {
            s2 = peg$parseChannelOperator();
            if (s2 === peg$FAILED) {
              s2 = peg$parseExpression();
              if (s2 === peg$FAILED) {
                s2 = peg$parseContainer();
                if (s2 === peg$FAILED) {
                  s2 = peg$parseNode();
                  if (s2 === peg$FAILED) {
                    s2 = [];
                    if (input.charCodeAt(peg$currPos) === 32) {
                      s3 = peg$c14;
                      peg$currPos++;
                    } else {
                      s3 = peg$FAILED;
                      if (peg$silentFails === 0) {
                        peg$fail(peg$c15);
                      }
                    }
                    if (s3 === peg$FAILED) {
                      if (peg$c2.test(input.charAt(peg$currPos))) {
                        s3 = input.charAt(peg$currPos);
                        peg$currPos++;
                      } else {
                        s3 = peg$FAILED;
                        if (peg$silentFails === 0) {
                          peg$fail(peg$c3);
                        }
                      }
                    }
                    while (s3 !== peg$FAILED) {
                      s2.push(s3);
                      if (input.charCodeAt(peg$currPos) === 32) {
                        s3 = peg$c14;
                        peg$currPos++;
                      } else {
                        s3 = peg$FAILED;
                        if (peg$silentFails === 0) {
                          peg$fail(peg$c15);
                        }
                      }
                      if (s3 === peg$FAILED) {
                        if (peg$c2.test(input.charAt(peg$currPos))) {
                          s3 = input.charAt(peg$currPos);
                          peg$currPos++;
                        } else {
                          s3 = peg$FAILED;
                          if (peg$silentFails === 0) {
                            peg$fail(peg$c3);
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = [];
            if (input.charCodeAt(peg$currPos) === 32) {
              s4 = peg$c14;
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c15);
              }
            }
            if (s4 === peg$FAILED) {
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s4 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
            }
            while (s4 !== peg$FAILED) {
              s3.push(s4);
              if (input.charCodeAt(peg$currPos) === 32) {
                s4 = peg$c14;
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c15);
                }
              }
              if (s4 === peg$FAILED) {
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s4 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s4 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
              }
            }
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c150(s1, s2, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c168);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseEssence() {
        var s0, s1, s2, s3, s4, s5;

        var key = peg$currPos * 68 + 39,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        s2 = peg$parseEssenceOpen();
        if (s2 !== peg$FAILED) {
          s3 = [];
          if (input.charCodeAt(peg$currPos) === 32) {
            s4 = peg$c14;
            peg$currPos++;
          } else {
            s4 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c15);
            }
          }
          if (s4 === peg$FAILED) {
            s4 = peg$currPos;
            if (peg$c12.test(input.charAt(peg$currPos))) {
              s5 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s5 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c13);
              }
            }
            if (s5 !== peg$FAILED) {
              peg$savedPos = s4;
              s5 = peg$c152(s2);
            }
            s4 = s5;
          }
          if (s4 === peg$FAILED) {
            s4 = peg$currPos;
            if (input.charCodeAt(peg$currPos) === 95) {
              s5 = peg$c22;
              peg$currPos++;
            } else {
              s5 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c23);
              }
            }
            if (s5 !== peg$FAILED) {
              peg$savedPos = s4;
              s5 = peg$c153(s2, s5);
            }
            s4 = s5;
            if (s4 === peg$FAILED) {
              s4 = peg$currPos;
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s5 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c152(s2);
              }
              s4 = s5;
            }
          }
          while (s4 !== peg$FAILED) {
            s3.push(s4);
            if (input.charCodeAt(peg$currPos) === 32) {
              s4 = peg$c14;
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c15);
              }
            }
            if (s4 === peg$FAILED) {
              s4 = peg$currPos;
              if (peg$c12.test(input.charAt(peg$currPos))) {
                s5 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c13);
                }
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c152(s2);
              }
              s4 = s5;
            }
            if (s4 === peg$FAILED) {
              s4 = peg$currPos;
              if (input.charCodeAt(peg$currPos) === 95) {
                s5 = peg$c22;
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c23);
                }
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c153(s2, s5);
              }
              s4 = s5;
              if (s4 === peg$FAILED) {
                s4 = peg$currPos;
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s5 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s5 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
                if (s5 !== peg$FAILED) {
                  peg$savedPos = s4;
                  s5 = peg$c152(s2);
                }
                s4 = s5;
              }
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseEssenceClose();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c154(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          s1 = peg$currPos;
          s2 = peg$parseEssenceOpen();
          if (s2 !== peg$FAILED) {
            s3 = peg$parseEssenceBody();
            if (s3 !== peg$FAILED) {
              s4 = peg$parseEssenceClose();
              if (s4 !== peg$FAILED) {
                peg$savedPos = s1;
                s2 = peg$c155(s2, s3, s4);
                s1 = s2;
              } else {
                peg$currPos = s1;
                s1 = peg$FAILED;
              }
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c170(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c169);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseConceptOpen() {
        var s0, s1, s2, s3, s4, s5;

        var key = peg$currPos * 68 + 40,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 60) {
          s1 = peg$c172;
          peg$currPos++;
        } else {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c173);
          }
        }
        if (s1 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s2 = peg$c22;
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = peg$currPos;
            s4 = peg$parseAnchorNode();
            if (s4 === peg$FAILED) {
              s4 = peg$parseConcept();
              if (s4 === peg$FAILED) {
                s4 = peg$parseLocation();
                if (s4 === peg$FAILED) {
                  s4 = peg$parseDomain();
                  if (s4 === peg$FAILED) {
                    s4 = peg$parseEssence();
                  }
                }
              }
            }
            if (s4 !== peg$FAILED) {
              s5 = peg$parseConcept();
              if (s5 === peg$FAILED) {
                s5 = peg$parseLocation();
                if (s5 === peg$FAILED) {
                  s5 = peg$parseDomain();
                  if (s5 === peg$FAILED) {
                    s5 = peg$parseEssence();
                  }
                }
              }
              if (s5 === peg$FAILED) {
                s5 = null;
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s3;
                s4 = peg$c139(s1, s4, s5);
                s3 = s4;
              } else {
                peg$currPos = s3;
                s3 = peg$FAILED;
              }
            } else {
              peg$currPos = s3;
              s3 = peg$FAILED;
            }
            if (s3 !== peg$FAILED) {
              s4 = peg$currPos;
              s5 = peg$parseSpace();
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c140(s1, s3);
              }
              s4 = s5;
              if (s4 !== peg$FAILED) {
                peg$savedPos = s0;
                s1 = peg$c174(s1, s3);
                s0 = s1;
              } else {
                peg$currPos = s0;
                s0 = peg$FAILED;
              }
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        if (s0 === peg$FAILED) {
          s0 = peg$currPos;
          if (input.charCodeAt(peg$currPos) === 60) {
            s1 = peg$c172;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c173);
            }
          }
          if (s1 !== peg$FAILED) {
            peg$savedPos = s0;
            s1 = peg$c175(s1);
          }
          s0 = s1;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c171);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseConceptClose() {
        var s0, s1, s2, s3;

        var key = peg$currPos * 68 + 41,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 62) {
          s1 = peg$c177;
          peg$currPos++;
        } else {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c178);
          }
        }
        if (s1 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s2 = peg$c22;
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = peg$parseAnchorNode();
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c179(s1, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        if (s0 === peg$FAILED) {
          s0 = peg$currPos;
          s1 = peg$parseAnchorNode();
          if (s1 !== peg$FAILED) {
            if (input.charCodeAt(peg$currPos) === 95) {
              s2 = peg$c22;
              peg$currPos++;
            } else {
              s2 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c23);
              }
            }
            if (s2 !== peg$FAILED) {
              if (input.charCodeAt(peg$currPos) === 62) {
                s3 = peg$c177;
                peg$currPos++;
              } else {
                s3 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c178);
                }
              }
              if (s3 !== peg$FAILED) {
                peg$savedPos = s0;
                s1 = peg$c180(s1, s3);
                s0 = s1;
              } else {
                peg$currPos = s0;
                s0 = peg$FAILED;
              }
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        }
        if (s0 === peg$FAILED) {
          s0 = peg$currPos;
          if (input.charCodeAt(peg$currPos) === 62) {
            s1 = peg$c177;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c178);
            }
          }
          if (s1 !== peg$FAILED) {
            peg$savedPos = s0;
            s1 = peg$c181(s1);
          }
          s0 = s1;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c176);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseConceptBody() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 42,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = [];
        if (input.charCodeAt(peg$currPos) === 32) {
          s2 = peg$c14;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c15);
          }
        }
        if (s2 === peg$FAILED) {
          if (peg$c2.test(input.charAt(peg$currPos))) {
            s2 = input.charAt(peg$currPos);
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c3);
            }
          }
        }
        while (s2 !== peg$FAILED) {
          s1.push(s2);
          if (input.charCodeAt(peg$currPos) === 32) {
            s2 = peg$c14;
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c15);
            }
          }
          if (s2 === peg$FAILED) {
            if (peg$c2.test(input.charAt(peg$currPos))) {
              s2 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s2 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c3);
              }
            }
          }
        }
        if (s1 !== peg$FAILED) {
          s2 = peg$parseBlockExpression();
          if (s2 === peg$FAILED) {
            s2 = peg$parseChannelOperator();
            if (s2 === peg$FAILED) {
              s2 = peg$parseExpression();
              if (s2 === peg$FAILED) {
                s2 = peg$parseContainer();
                if (s2 === peg$FAILED) {
                  s2 = peg$parseNode();
                  if (s2 === peg$FAILED) {
                    s2 = [];
                    if (input.charCodeAt(peg$currPos) === 32) {
                      s3 = peg$c14;
                      peg$currPos++;
                    } else {
                      s3 = peg$FAILED;
                      if (peg$silentFails === 0) {
                        peg$fail(peg$c15);
                      }
                    }
                    if (s3 === peg$FAILED) {
                      if (peg$c2.test(input.charAt(peg$currPos))) {
                        s3 = input.charAt(peg$currPos);
                        peg$currPos++;
                      } else {
                        s3 = peg$FAILED;
                        if (peg$silentFails === 0) {
                          peg$fail(peg$c3);
                        }
                      }
                    }
                    while (s3 !== peg$FAILED) {
                      s2.push(s3);
                      if (input.charCodeAt(peg$currPos) === 32) {
                        s3 = peg$c14;
                        peg$currPos++;
                      } else {
                        s3 = peg$FAILED;
                        if (peg$silentFails === 0) {
                          peg$fail(peg$c15);
                        }
                      }
                      if (s3 === peg$FAILED) {
                        if (peg$c2.test(input.charAt(peg$currPos))) {
                          s3 = input.charAt(peg$currPos);
                          peg$currPos++;
                        } else {
                          s3 = peg$FAILED;
                          if (peg$silentFails === 0) {
                            peg$fail(peg$c3);
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = [];
            if (input.charCodeAt(peg$currPos) === 32) {
              s4 = peg$c14;
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c15);
              }
            }
            if (s4 === peg$FAILED) {
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s4 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
            }
            while (s4 !== peg$FAILED) {
              s3.push(s4);
              if (input.charCodeAt(peg$currPos) === 32) {
                s4 = peg$c14;
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c15);
                }
              }
              if (s4 === peg$FAILED) {
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s4 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s4 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
              }
            }
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c150(s1, s2, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c182);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseConcept() {
        var s0, s1, s2, s3, s4, s5;

        var key = peg$currPos * 68 + 43,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        s2 = peg$parseConceptOpen();
        if (s2 !== peg$FAILED) {
          s3 = [];
          if (input.charCodeAt(peg$currPos) === 32) {
            s4 = peg$c14;
            peg$currPos++;
          } else {
            s4 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c15);
            }
          }
          if (s4 === peg$FAILED) {
            s4 = peg$currPos;
            if (peg$c12.test(input.charAt(peg$currPos))) {
              s5 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s5 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c13);
              }
            }
            if (s5 !== peg$FAILED) {
              peg$savedPos = s4;
              s5 = peg$c152(s2);
            }
            s4 = s5;
          }
          if (s4 === peg$FAILED) {
            s4 = peg$currPos;
            if (input.charCodeAt(peg$currPos) === 95) {
              s5 = peg$c22;
              peg$currPos++;
            } else {
              s5 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c23);
              }
            }
            if (s5 !== peg$FAILED) {
              peg$savedPos = s4;
              s5 = peg$c153(s2, s5);
            }
            s4 = s5;
            if (s4 === peg$FAILED) {
              s4 = peg$currPos;
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s5 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c152(s2);
              }
              s4 = s5;
            }
          }
          while (s4 !== peg$FAILED) {
            s3.push(s4);
            if (input.charCodeAt(peg$currPos) === 32) {
              s4 = peg$c14;
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c15);
              }
            }
            if (s4 === peg$FAILED) {
              s4 = peg$currPos;
              if (peg$c12.test(input.charAt(peg$currPos))) {
                s5 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c13);
                }
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c152(s2);
              }
              s4 = s5;
            }
            if (s4 === peg$FAILED) {
              s4 = peg$currPos;
              if (input.charCodeAt(peg$currPos) === 95) {
                s5 = peg$c22;
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c23);
                }
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c153(s2, s5);
              }
              s4 = s5;
              if (s4 === peg$FAILED) {
                s4 = peg$currPos;
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s5 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s5 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
                if (s5 !== peg$FAILED) {
                  peg$savedPos = s4;
                  s5 = peg$c152(s2);
                }
                s4 = s5;
              }
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseConceptClose();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c154(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          s1 = peg$currPos;
          s2 = peg$parseConceptOpen();
          if (s2 !== peg$FAILED) {
            s3 = peg$parseConceptBody();
            if (s3 !== peg$FAILED) {
              s4 = peg$parseConceptClose();
              if (s4 !== peg$FAILED) {
                peg$savedPos = s1;
                s2 = peg$c155(s2, s3, s4);
                s1 = s2;
              } else {
                peg$currPos = s1;
                s1 = peg$FAILED;
              }
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c184(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c183);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseLocationOpen() {
        var s0, s1, s2, s3, s4, s5;

        var key = peg$currPos * 68 + 44,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 40) {
          s1 = peg$c186;
          peg$currPos++;
        } else {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c187);
          }
        }
        if (s1 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s2 = peg$c22;
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = peg$currPos;
            s4 = peg$parseAnchorNode();
            if (s4 === peg$FAILED) {
              s4 = peg$parseConcept();
              if (s4 === peg$FAILED) {
                s4 = peg$parseLocation();
                if (s4 === peg$FAILED) {
                  s4 = peg$parseDomain();
                  if (s4 === peg$FAILED) {
                    s4 = peg$parseEssence();
                  }
                }
              }
            }
            if (s4 !== peg$FAILED) {
              s5 = peg$parseConcept();
              if (s5 === peg$FAILED) {
                s5 = peg$parseLocation();
                if (s5 === peg$FAILED) {
                  s5 = peg$parseDomain();
                  if (s5 === peg$FAILED) {
                    s5 = peg$parseEssence();
                  }
                }
              }
              if (s5 === peg$FAILED) {
                s5 = null;
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s3;
                s4 = peg$c139(s1, s4, s5);
                s3 = s4;
              } else {
                peg$currPos = s3;
                s3 = peg$FAILED;
              }
            } else {
              peg$currPos = s3;
              s3 = peg$FAILED;
            }
            if (s3 !== peg$FAILED) {
              s4 = peg$currPos;
              s5 = peg$parseSpace();
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c140(s1, s3);
              }
              s4 = s5;
              if (s4 !== peg$FAILED) {
                peg$savedPos = s0;
                s1 = peg$c188(s1, s3);
                s0 = s1;
              } else {
                peg$currPos = s0;
                s0 = peg$FAILED;
              }
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        if (s0 === peg$FAILED) {
          s0 = peg$currPos;
          if (input.charCodeAt(peg$currPos) === 40) {
            s1 = peg$c186;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c187);
            }
          }
          if (s1 !== peg$FAILED) {
            peg$savedPos = s0;
            s1 = peg$c189(s1);
          }
          s0 = s1;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c185);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseLocationClose() {
        var s0, s1, s2, s3;

        var key = peg$currPos * 68 + 45,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        if (input.charCodeAt(peg$currPos) === 41) {
          s1 = peg$c191;
          peg$currPos++;
        } else {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c192);
          }
        }
        if (s1 !== peg$FAILED) {
          if (input.charCodeAt(peg$currPos) === 95) {
            s2 = peg$c22;
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c23);
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = peg$parseAnchorNode();
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c193(s1, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        if (s0 === peg$FAILED) {
          s0 = peg$currPos;
          s1 = peg$parseAnchorNode();
          if (s1 !== peg$FAILED) {
            if (input.charCodeAt(peg$currPos) === 95) {
              s2 = peg$c22;
              peg$currPos++;
            } else {
              s2 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c23);
              }
            }
            if (s2 !== peg$FAILED) {
              if (input.charCodeAt(peg$currPos) === 41) {
                s3 = peg$c191;
                peg$currPos++;
              } else {
                s3 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c192);
                }
              }
              if (s3 !== peg$FAILED) {
                peg$savedPos = s0;
                s1 = peg$c194(s1, s3);
                s0 = s1;
              } else {
                peg$currPos = s0;
                s0 = peg$FAILED;
              }
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        }
        if (s0 === peg$FAILED) {
          s0 = peg$currPos;
          if (input.charCodeAt(peg$currPos) === 41) {
            s1 = peg$c191;
            peg$currPos++;
          } else {
            s1 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c192);
            }
          }
          if (s1 !== peg$FAILED) {
            peg$savedPos = s0;
            s1 = peg$c195(s1);
          }
          s0 = s1;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c190);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseLocationBody() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 46,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = [];
        if (input.charCodeAt(peg$currPos) === 32) {
          s2 = peg$c14;
          peg$currPos++;
        } else {
          s2 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c15);
          }
        }
        if (s2 === peg$FAILED) {
          if (peg$c2.test(input.charAt(peg$currPos))) {
            s2 = input.charAt(peg$currPos);
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c3);
            }
          }
        }
        while (s2 !== peg$FAILED) {
          s1.push(s2);
          if (input.charCodeAt(peg$currPos) === 32) {
            s2 = peg$c14;
            peg$currPos++;
          } else {
            s2 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c15);
            }
          }
          if (s2 === peg$FAILED) {
            if (peg$c2.test(input.charAt(peg$currPos))) {
              s2 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s2 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c3);
              }
            }
          }
        }
        if (s1 !== peg$FAILED) {
          s2 = peg$parseBlockExpression();
          if (s2 === peg$FAILED) {
            s2 = peg$parseChannelOperator();
            if (s2 === peg$FAILED) {
              s2 = peg$parseExpression();
              if (s2 === peg$FAILED) {
                s2 = peg$parseContainer();
                if (s2 === peg$FAILED) {
                  s2 = peg$parseNode();
                  if (s2 === peg$FAILED) {
                    s2 = [];
                    if (input.charCodeAt(peg$currPos) === 32) {
                      s3 = peg$c14;
                      peg$currPos++;
                    } else {
                      s3 = peg$FAILED;
                      if (peg$silentFails === 0) {
                        peg$fail(peg$c15);
                      }
                    }
                    if (s3 === peg$FAILED) {
                      if (peg$c2.test(input.charAt(peg$currPos))) {
                        s3 = input.charAt(peg$currPos);
                        peg$currPos++;
                      } else {
                        s3 = peg$FAILED;
                        if (peg$silentFails === 0) {
                          peg$fail(peg$c3);
                        }
                      }
                    }
                    while (s3 !== peg$FAILED) {
                      s2.push(s3);
                      if (input.charCodeAt(peg$currPos) === 32) {
                        s3 = peg$c14;
                        peg$currPos++;
                      } else {
                        s3 = peg$FAILED;
                        if (peg$silentFails === 0) {
                          peg$fail(peg$c15);
                        }
                      }
                      if (s3 === peg$FAILED) {
                        if (peg$c2.test(input.charAt(peg$currPos))) {
                          s3 = input.charAt(peg$currPos);
                          peg$currPos++;
                        } else {
                          s3 = peg$FAILED;
                          if (peg$silentFails === 0) {
                            peg$fail(peg$c3);
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = [];
            if (input.charCodeAt(peg$currPos) === 32) {
              s4 = peg$c14;
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c15);
              }
            }
            if (s4 === peg$FAILED) {
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s4 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
            }
            while (s4 !== peg$FAILED) {
              s3.push(s4);
              if (input.charCodeAt(peg$currPos) === 32) {
                s4 = peg$c14;
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c15);
                }
              }
              if (s4 === peg$FAILED) {
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s4 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s4 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
              }
            }
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c150(s1, s2, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c196);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseLocation() {
        var s0, s1, s2, s3, s4, s5;

        var key = peg$currPos * 68 + 47,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        s2 = peg$parseLocationOpen();
        if (s2 !== peg$FAILED) {
          s3 = [];
          if (input.charCodeAt(peg$currPos) === 32) {
            s4 = peg$c14;
            peg$currPos++;
          } else {
            s4 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c15);
            }
          }
          if (s4 === peg$FAILED) {
            s4 = peg$currPos;
            if (peg$c12.test(input.charAt(peg$currPos))) {
              s5 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s5 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c13);
              }
            }
            if (s5 !== peg$FAILED) {
              peg$savedPos = s4;
              s5 = peg$c152(s2);
            }
            s4 = s5;
          }
          if (s4 === peg$FAILED) {
            s4 = peg$currPos;
            if (input.charCodeAt(peg$currPos) === 95) {
              s5 = peg$c22;
              peg$currPos++;
            } else {
              s5 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c23);
              }
            }
            if (s5 !== peg$FAILED) {
              peg$savedPos = s4;
              s5 = peg$c153(s2, s5);
            }
            s4 = s5;
            if (s4 === peg$FAILED) {
              s4 = peg$currPos;
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s5 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c152(s2);
              }
              s4 = s5;
            }
          }
          while (s4 !== peg$FAILED) {
            s3.push(s4);
            if (input.charCodeAt(peg$currPos) === 32) {
              s4 = peg$c14;
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c15);
              }
            }
            if (s4 === peg$FAILED) {
              s4 = peg$currPos;
              if (peg$c12.test(input.charAt(peg$currPos))) {
                s5 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c13);
                }
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c152(s2);
              }
              s4 = s5;
            }
            if (s4 === peg$FAILED) {
              s4 = peg$currPos;
              if (input.charCodeAt(peg$currPos) === 95) {
                s5 = peg$c22;
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c23);
                }
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s4;
                s5 = peg$c153(s2, s5);
              }
              s4 = s5;
              if (s4 === peg$FAILED) {
                s4 = peg$currPos;
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s5 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s5 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
                if (s5 !== peg$FAILED) {
                  peg$savedPos = s4;
                  s5 = peg$c152(s2);
                }
                s4 = s5;
              }
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseLocationClose();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c154(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          s1 = peg$currPos;
          s2 = peg$parseLocationOpen();
          if (s2 !== peg$FAILED) {
            s3 = peg$parseLocationBody();
            if (s3 !== peg$FAILED) {
              s4 = peg$parseLocationClose();
              if (s4 !== peg$FAILED) {
                peg$savedPos = s1;
                s2 = peg$c155(s2, s3, s4);
                s1 = s2;
              } else {
                peg$currPos = s1;
                s1 = peg$FAILED;
              }
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c198(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c197);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseExpression() {
        var s0, s1;

        var key = peg$currPos * 68 + 48,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$parseInfixExpression();
        if (s0 === peg$FAILED) {
          s0 = peg$parseSequenceExpression();
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c199);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseBlockExpression() {
        var s0, s1, s2, s3, s4, s5, s6, s7, s8, s9;

        var key = peg$currPos * 68 + 49,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$currPos;
        s2 = [];
        s3 = peg$currPos;
        s4 = peg$parseExpression();
        if (s4 === peg$FAILED) {
          s4 = peg$parseContainer();
          if (s4 === peg$FAILED) {
            s4 = peg$parseNode();
            if (s4 === peg$FAILED) {
              s4 = peg$parseChannelOperator();
            }
          }
        }
        if (s4 !== peg$FAILED) {
          s5 = [];
          s6 = peg$currPos;
          s7 = peg$parseSpace();
          if (s7 !== peg$FAILED) {
            peg$savedPos = s6;
            s7 = peg$c201(s4);
          }
          s6 = s7;
          if (s6 === peg$FAILED) {
            if (peg$c2.test(input.charAt(peg$currPos))) {
              s6 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s6 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c3);
              }
            }
          }
          while (s6 !== peg$FAILED) {
            s5.push(s6);
            s6 = peg$currPos;
            s7 = peg$parseSpace();
            if (s7 !== peg$FAILED) {
              peg$savedPos = s6;
              s7 = peg$c201(s4);
            }
            s6 = s7;
            if (s6 === peg$FAILED) {
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s6 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s6 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
            }
          }
          if (s5 !== peg$FAILED) {
            s6 = peg$parseBlockDelimiter();
            if (s6 !== peg$FAILED) {
              s7 = [];
              s8 = peg$currPos;
              s9 = peg$parseSpace();
              if (s9 !== peg$FAILED) {
                peg$savedPos = s8;
                s9 = peg$c202(s4, s6);
              }
              s8 = s9;
              if (s8 === peg$FAILED) {
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s8 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s8 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
              }
              while (s8 !== peg$FAILED) {
                s7.push(s8);
                s8 = peg$currPos;
                s9 = peg$parseSpace();
                if (s9 !== peg$FAILED) {
                  peg$savedPos = s8;
                  s9 = peg$c202(s4, s6);
                }
                s8 = s9;
                if (s8 === peg$FAILED) {
                  if (peg$c2.test(input.charAt(peg$currPos))) {
                    s8 = input.charAt(peg$currPos);
                    peg$currPos++;
                  } else {
                    s8 = peg$FAILED;
                    if (peg$silentFails === 0) {
                      peg$fail(peg$c3);
                    }
                  }
                }
              }
              if (s7 !== peg$FAILED) {
                peg$savedPos = s3;
                s4 = peg$c203(s4, s6);
                s3 = s4;
              } else {
                peg$currPos = s3;
                s3 = peg$FAILED;
              }
            } else {
              peg$currPos = s3;
              s3 = peg$FAILED;
            }
          } else {
            peg$currPos = s3;
            s3 = peg$FAILED;
          }
        } else {
          peg$currPos = s3;
          s3 = peg$FAILED;
        }
        if (s3 !== peg$FAILED) {
          while (s3 !== peg$FAILED) {
            s2.push(s3);
            s3 = peg$currPos;
            s4 = peg$parseExpression();
            if (s4 === peg$FAILED) {
              s4 = peg$parseContainer();
              if (s4 === peg$FAILED) {
                s4 = peg$parseNode();
                if (s4 === peg$FAILED) {
                  s4 = peg$parseChannelOperator();
                }
              }
            }
            if (s4 !== peg$FAILED) {
              s5 = [];
              s6 = peg$currPos;
              s7 = peg$parseSpace();
              if (s7 !== peg$FAILED) {
                peg$savedPos = s6;
                s7 = peg$c201(s4);
              }
              s6 = s7;
              if (s6 === peg$FAILED) {
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s6 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s6 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
              }
              while (s6 !== peg$FAILED) {
                s5.push(s6);
                s6 = peg$currPos;
                s7 = peg$parseSpace();
                if (s7 !== peg$FAILED) {
                  peg$savedPos = s6;
                  s7 = peg$c201(s4);
                }
                s6 = s7;
                if (s6 === peg$FAILED) {
                  if (peg$c2.test(input.charAt(peg$currPos))) {
                    s6 = input.charAt(peg$currPos);
                    peg$currPos++;
                  } else {
                    s6 = peg$FAILED;
                    if (peg$silentFails === 0) {
                      peg$fail(peg$c3);
                    }
                  }
                }
              }
              if (s5 !== peg$FAILED) {
                s6 = peg$parseBlockDelimiter();
                if (s6 !== peg$FAILED) {
                  s7 = [];
                  s8 = peg$currPos;
                  s9 = peg$parseSpace();
                  if (s9 !== peg$FAILED) {
                    peg$savedPos = s8;
                    s9 = peg$c202(s4, s6);
                  }
                  s8 = s9;
                  if (s8 === peg$FAILED) {
                    if (peg$c2.test(input.charAt(peg$currPos))) {
                      s8 = input.charAt(peg$currPos);
                      peg$currPos++;
                    } else {
                      s8 = peg$FAILED;
                      if (peg$silentFails === 0) {
                        peg$fail(peg$c3);
                      }
                    }
                  }
                  while (s8 !== peg$FAILED) {
                    s7.push(s8);
                    s8 = peg$currPos;
                    s9 = peg$parseSpace();
                    if (s9 !== peg$FAILED) {
                      peg$savedPos = s8;
                      s9 = peg$c202(s4, s6);
                    }
                    s8 = s9;
                    if (s8 === peg$FAILED) {
                      if (peg$c2.test(input.charAt(peg$currPos))) {
                        s8 = input.charAt(peg$currPos);
                        peg$currPos++;
                      } else {
                        s8 = peg$FAILED;
                        if (peg$silentFails === 0) {
                          peg$fail(peg$c3);
                        }
                      }
                    }
                  }
                  if (s7 !== peg$FAILED) {
                    peg$savedPos = s3;
                    s4 = peg$c203(s4, s6);
                    s3 = s4;
                  } else {
                    peg$currPos = s3;
                    s3 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s3;
                  s3 = peg$FAILED;
                }
              } else {
                peg$currPos = s3;
                s3 = peg$FAILED;
              }
            } else {
              peg$currPos = s3;
              s3 = peg$FAILED;
            }
          }
        } else {
          s2 = peg$FAILED;
        }
        if (s2 !== peg$FAILED) {
          s3 = [];
          s4 = peg$currPos;
          s5 = peg$parseSpace();
          if (s5 !== peg$FAILED) {
            peg$savedPos = s4;
            s5 = peg$c204(s2);
          }
          s4 = s5;
          if (s4 === peg$FAILED) {
            if (peg$c2.test(input.charAt(peg$currPos))) {
              s4 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s4 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c3);
              }
            }
          }
          while (s4 !== peg$FAILED) {
            s3.push(s4);
            s4 = peg$currPos;
            s5 = peg$parseSpace();
            if (s5 !== peg$FAILED) {
              peg$savedPos = s4;
              s5 = peg$c204(s2);
            }
            s4 = s5;
            if (s4 === peg$FAILED) {
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s4 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s4 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseExpression();
            if (s4 === peg$FAILED) {
              s4 = peg$parseContainer();
              if (s4 === peg$FAILED) {
                s4 = peg$parseNode();
                if (s4 === peg$FAILED) {
                  s4 = peg$parseChannelOperator();
                }
              }
            }
            if (s4 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c205(s2, s4);
              s1 = s2;
            } else {
              peg$currPos = s1;
              s1 = peg$FAILED;
            }
          } else {
            peg$currPos = s1;
            s1 = peg$FAILED;
          }
        } else {
          peg$currPos = s1;
          s1 = peg$FAILED;
        }
        if (s1 === peg$FAILED) {
          s1 = peg$currPos;
          s2 = [];
          s3 = peg$currPos;
          s4 = peg$parseExpression();
          if (s4 === peg$FAILED) {
            s4 = peg$parseContainer();
            if (s4 === peg$FAILED) {
              s4 = peg$parseNode();
              if (s4 === peg$FAILED) {
                s4 = peg$parseChannelOperator();
              }
            }
          }
          if (s4 !== peg$FAILED) {
            s5 = [];
            s6 = peg$currPos;
            s7 = peg$parseSpace();
            if (s7 !== peg$FAILED) {
              peg$savedPos = s6;
              s7 = peg$c201(s4);
            }
            s6 = s7;
            if (s6 === peg$FAILED) {
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s6 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s6 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
            }
            while (s6 !== peg$FAILED) {
              s5.push(s6);
              s6 = peg$currPos;
              s7 = peg$parseSpace();
              if (s7 !== peg$FAILED) {
                peg$savedPos = s6;
                s7 = peg$c201(s4);
              }
              s6 = s7;
              if (s6 === peg$FAILED) {
                if (peg$c2.test(input.charAt(peg$currPos))) {
                  s6 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s6 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c3);
                  }
                }
              }
            }
            if (s5 !== peg$FAILED) {
              s6 = peg$parseBlockDelimiter();
              if (s6 !== peg$FAILED) {
                s7 = [];
                s8 = peg$currPos;
                s9 = peg$parseSpace();
                if (s9 !== peg$FAILED) {
                  peg$savedPos = s8;
                  s9 = peg$c202(s4, s6);
                }
                s8 = s9;
                if (s8 === peg$FAILED) {
                  if (peg$c2.test(input.charAt(peg$currPos))) {
                    s8 = input.charAt(peg$currPos);
                    peg$currPos++;
                  } else {
                    s8 = peg$FAILED;
                    if (peg$silentFails === 0) {
                      peg$fail(peg$c3);
                    }
                  }
                }
                while (s8 !== peg$FAILED) {
                  s7.push(s8);
                  s8 = peg$currPos;
                  s9 = peg$parseSpace();
                  if (s9 !== peg$FAILED) {
                    peg$savedPos = s8;
                    s9 = peg$c202(s4, s6);
                  }
                  s8 = s9;
                  if (s8 === peg$FAILED) {
                    if (peg$c2.test(input.charAt(peg$currPos))) {
                      s8 = input.charAt(peg$currPos);
                      peg$currPos++;
                    } else {
                      s8 = peg$FAILED;
                      if (peg$silentFails === 0) {
                        peg$fail(peg$c3);
                      }
                    }
                  }
                }
                if (s7 !== peg$FAILED) {
                  peg$savedPos = s3;
                  s4 = peg$c203(s4, s6);
                  s3 = s4;
                } else {
                  peg$currPos = s3;
                  s3 = peg$FAILED;
                }
              } else {
                peg$currPos = s3;
                s3 = peg$FAILED;
              }
            } else {
              peg$currPos = s3;
              s3 = peg$FAILED;
            }
          } else {
            peg$currPos = s3;
            s3 = peg$FAILED;
          }
          if (s3 !== peg$FAILED) {
            while (s3 !== peg$FAILED) {
              s2.push(s3);
              s3 = peg$currPos;
              s4 = peg$parseExpression();
              if (s4 === peg$FAILED) {
                s4 = peg$parseContainer();
                if (s4 === peg$FAILED) {
                  s4 = peg$parseNode();
                  if (s4 === peg$FAILED) {
                    s4 = peg$parseChannelOperator();
                  }
                }
              }
              if (s4 !== peg$FAILED) {
                s5 = [];
                s6 = peg$currPos;
                s7 = peg$parseSpace();
                if (s7 !== peg$FAILED) {
                  peg$savedPos = s6;
                  s7 = peg$c201(s4);
                }
                s6 = s7;
                if (s6 === peg$FAILED) {
                  if (peg$c2.test(input.charAt(peg$currPos))) {
                    s6 = input.charAt(peg$currPos);
                    peg$currPos++;
                  } else {
                    s6 = peg$FAILED;
                    if (peg$silentFails === 0) {
                      peg$fail(peg$c3);
                    }
                  }
                }
                while (s6 !== peg$FAILED) {
                  s5.push(s6);
                  s6 = peg$currPos;
                  s7 = peg$parseSpace();
                  if (s7 !== peg$FAILED) {
                    peg$savedPos = s6;
                    s7 = peg$c201(s4);
                  }
                  s6 = s7;
                  if (s6 === peg$FAILED) {
                    if (peg$c2.test(input.charAt(peg$currPos))) {
                      s6 = input.charAt(peg$currPos);
                      peg$currPos++;
                    } else {
                      s6 = peg$FAILED;
                      if (peg$silentFails === 0) {
                        peg$fail(peg$c3);
                      }
                    }
                  }
                }
                if (s5 !== peg$FAILED) {
                  s6 = peg$parseBlockDelimiter();
                  if (s6 !== peg$FAILED) {
                    s7 = [];
                    s8 = peg$currPos;
                    s9 = peg$parseSpace();
                    if (s9 !== peg$FAILED) {
                      peg$savedPos = s8;
                      s9 = peg$c202(s4, s6);
                    }
                    s8 = s9;
                    if (s8 === peg$FAILED) {
                      if (peg$c2.test(input.charAt(peg$currPos))) {
                        s8 = input.charAt(peg$currPos);
                        peg$currPos++;
                      } else {
                        s8 = peg$FAILED;
                        if (peg$silentFails === 0) {
                          peg$fail(peg$c3);
                        }
                      }
                    }
                    while (s8 !== peg$FAILED) {
                      s7.push(s8);
                      s8 = peg$currPos;
                      s9 = peg$parseSpace();
                      if (s9 !== peg$FAILED) {
                        peg$savedPos = s8;
                        s9 = peg$c202(s4, s6);
                      }
                      s8 = s9;
                      if (s8 === peg$FAILED) {
                        if (peg$c2.test(input.charAt(peg$currPos))) {
                          s8 = input.charAt(peg$currPos);
                          peg$currPos++;
                        } else {
                          s8 = peg$FAILED;
                          if (peg$silentFails === 0) {
                            peg$fail(peg$c3);
                          }
                        }
                      }
                    }
                    if (s7 !== peg$FAILED) {
                      peg$savedPos = s3;
                      s4 = peg$c203(s4, s6);
                      s3 = s4;
                    } else {
                      peg$currPos = s3;
                      s3 = peg$FAILED;
                    }
                  } else {
                    peg$currPos = s3;
                    s3 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s3;
                  s3 = peg$FAILED;
                }
              } else {
                peg$currPos = s3;
                s3 = peg$FAILED;
              }
            }
          } else {
            s2 = peg$FAILED;
          }
          if (s2 !== peg$FAILED) {
            peg$savedPos = s1;
            s2 = peg$c206(s2);
          }
          s1 = s2;
          if (s1 === peg$FAILED) {
            s1 = peg$currPos;
            s2 = peg$parseExpression();
            if (s2 === peg$FAILED) {
              s2 = peg$parseContainer();
              if (s2 === peg$FAILED) {
                s2 = peg$parseNode();
                if (s2 === peg$FAILED) {
                  s2 = peg$parseChannelOperator();
                }
              }
            }
            if (s2 !== peg$FAILED) {
              peg$savedPos = s1;
              s2 = peg$c207(s2);
            }
            s1 = s2;
          }
        }
        if (s1 !== peg$FAILED) {
          peg$savedPos = s0;
          s1 = peg$c208(s1);
        }
        s0 = s1;
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c200);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseInstanceExpression() {
        var s0, s1, s2, s3;

        var key = peg$currPos * 68 + 50,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseEntityExpression();
        if (s1 !== peg$FAILED) {
          s2 = [];
          if (input.charCodeAt(peg$currPos) === 32) {
            s3 = peg$c14;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c15);
            }
          }
          if (s3 === peg$FAILED) {
            if (peg$c2.test(input.charAt(peg$currPos))) {
              s3 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s3 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c3);
              }
            }
          }
          while (s3 !== peg$FAILED) {
            s2.push(s3);
            if (input.charCodeAt(peg$currPos) === 32) {
              s3 = peg$c14;
              peg$currPos++;
            } else {
              s3 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c15);
              }
            }
            if (s3 === peg$FAILED) {
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s3 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s3 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = peg$parseBehaviorExpression();
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c210(s1, s2, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c209);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseBehaviorExpression() {
        var s0, s1, s2, s3, s4, s5;

        var key = peg$currPos * 68 + 51,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseLocation();
        if (s1 !== peg$FAILED) {
          s2 = peg$parseDomain();
          if (s2 !== peg$FAILED) {
            s3 = peg$parseEssence();
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c212(s1, s2, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        if (s0 === peg$FAILED) {
          s0 = peg$currPos;
          s1 = peg$parseLocation();
          if (s1 !== peg$FAILED) {
            s2 = [];
            if (input.charCodeAt(peg$currPos) === 32) {
              s3 = peg$c14;
              peg$currPos++;
            } else {
              s3 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c15);
              }
            }
            while (s3 !== peg$FAILED) {
              s2.push(s3);
              if (input.charCodeAt(peg$currPos) === 32) {
                s3 = peg$c14;
                peg$currPos++;
              } else {
                s3 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c15);
                }
              }
            }
            if (s2 !== peg$FAILED) {
              s3 = peg$parseDomain();
              if (s3 !== peg$FAILED) {
                s4 = [];
                if (input.charCodeAt(peg$currPos) === 32) {
                  s5 = peg$c14;
                  peg$currPos++;
                } else {
                  s5 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c15);
                  }
                }
                while (s5 !== peg$FAILED) {
                  s4.push(s5);
                  if (input.charCodeAt(peg$currPos) === 32) {
                    s5 = peg$c14;
                    peg$currPos++;
                  } else {
                    s5 = peg$FAILED;
                    if (peg$silentFails === 0) {
                      peg$fail(peg$c15);
                    }
                  }
                }
                if (s4 !== peg$FAILED) {
                  s5 = peg$parseEssence();
                  if (s5 !== peg$FAILED) {
                    peg$savedPos = s0;
                    s1 = peg$c212(s1, s3, s5);
                    s0 = s1;
                  } else {
                    peg$currPos = s0;
                    s0 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s0;
                  s0 = peg$FAILED;
                }
              } else {
                peg$currPos = s0;
                s0 = peg$FAILED;
              }
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
          if (s0 === peg$FAILED) {
            s0 = peg$currPos;
            s1 = peg$parseDomain();
            if (s1 !== peg$FAILED) {
              s2 = [];
              if (input.charCodeAt(peg$currPos) === 32) {
                s3 = peg$c14;
                peg$currPos++;
              } else {
                s3 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c15);
                }
              }
              while (s3 !== peg$FAILED) {
                s2.push(s3);
                if (input.charCodeAt(peg$currPos) === 32) {
                  s3 = peg$c14;
                  peg$currPos++;
                } else {
                  s3 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c15);
                  }
                }
              }
              if (s2 !== peg$FAILED) {
                s3 = peg$parseEssence();
                if (s3 !== peg$FAILED) {
                  peg$savedPos = s0;
                  s1 = peg$c213(s1, s3);
                  s0 = s1;
                } else {
                  peg$currPos = s0;
                  s0 = peg$FAILED;
                }
              } else {
                peg$currPos = s0;
                s0 = peg$FAILED;
              }
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          }
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c211);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseEntityExpression() {
        var s0, s1, s2, s3;

        var key = peg$currPos * 68 + 52,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseConcept();
        if (s1 !== peg$FAILED) {
          s2 = [];
          if (input.charCodeAt(peg$currPos) === 32) {
            s3 = peg$c14;
            peg$currPos++;
          } else {
            s3 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c15);
            }
          }
          while (s3 !== peg$FAILED) {
            s2.push(s3);
            if (input.charCodeAt(peg$currPos) === 32) {
              s3 = peg$c14;
              peg$currPos++;
            } else {
              s3 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c15);
              }
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = peg$parseScalar();
            if (s3 === peg$FAILED) {
              s3 = peg$parseReferenceOperator();
            }
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c215(s1, s2, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        if (s0 === peg$FAILED) {
          s0 = peg$currPos;
          s1 = peg$parseScalar();
          if (s1 === peg$FAILED) {
            s1 = peg$parseReferenceOperator();
          }
          if (s1 !== peg$FAILED) {
            peg$savedPos = s0;
            s1 = peg$c216(s1);
          }
          s0 = s1;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c214);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseLocatedConceptExpression() {
        var s0, s1, s2;

        var key = peg$currPos * 68 + 53,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseLocation();
        if (s1 !== peg$FAILED) {
          s2 = peg$parseConcept();
          if (s2 !== peg$FAILED) {
            peg$savedPos = s0;
            s1 = peg$c218(s1, s2);
            s0 = s1;
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c217);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseLocatedDomainExpression() {
        var s0, s1, s2;

        var key = peg$currPos * 68 + 54,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseLocation();
        if (s1 !== peg$FAILED) {
          s2 = peg$parseDomain();
          if (s2 !== peg$FAILED) {
            peg$savedPos = s0;
            s1 = peg$c220(s1, s2);
            s0 = s1;
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c219);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseLocatedEntityExpression() {
        var s0, s1, s2;

        var key = peg$currPos * 68 + 55,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseLocation();
        if (s1 !== peg$FAILED) {
          s2 = peg$parseEntityExpression();
          if (s2 !== peg$FAILED) {
            peg$savedPos = s0;
            s1 = peg$c222(s1, s2);
            s0 = s1;
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c221);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseLocatedEssenceExpression() {
        var s0, s1, s2;

        var key = peg$currPos * 68 + 56,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseLocation();
        if (s1 !== peg$FAILED) {
          s2 = peg$parseEssence();
          if (s2 !== peg$FAILED) {
            peg$savedPos = s0;
            s1 = peg$c224(s1, s2);
            s0 = s1;
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c223);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseSequenceExpression() {
        var s0, s1;

        var key = peg$currPos * 68 + 57,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$parseInstanceExpression();
        if (s0 === peg$FAILED) {
          s0 = peg$parseEntityExpression();
          if (s0 === peg$FAILED) {
            s0 = peg$parseBehaviorExpression();
            if (s0 === peg$FAILED) {
              s0 = peg$parseLocatedEntityExpression();
              if (s0 === peg$FAILED) {
                s0 = peg$parseLocatedConceptExpression();
                if (s0 === peg$FAILED) {
                  s0 = peg$parseLocatedEssenceExpression();
                  if (s0 === peg$FAILED) {
                    s0 = peg$parseLocatedDomainExpression();
                  }
                }
              }
            }
          }
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c225);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseCommonExpression() {
        var s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10;

        var key = peg$currPos * 68 + 58,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseNode();
        if (s1 !== peg$FAILED) {
          s2 = [];
          s3 = peg$currPos;
          s4 = peg$parseSpace();
          if (s4 !== peg$FAILED) {
            peg$savedPos = s3;
            s4 = peg$c204(s1);
          }
          s3 = s4;
          while (s3 !== peg$FAILED) {
            s2.push(s3);
            s3 = peg$currPos;
            s4 = peg$parseSpace();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s3;
              s4 = peg$c204(s1);
            }
            s3 = s4;
          }
          if (s2 !== peg$FAILED) {
            s3 = [];
            s4 = peg$currPos;
            s5 = peg$parseCommonDelimiter();
            if (s5 !== peg$FAILED) {
              s6 = [];
              s7 = peg$currPos;
              s8 = peg$parseSpace();
              if (s8 !== peg$FAILED) {
                peg$savedPos = s7;
                s8 = peg$c227(s1, s5);
              }
              s7 = s8;
              while (s7 !== peg$FAILED) {
                s6.push(s7);
                s7 = peg$currPos;
                s8 = peg$parseSpace();
                if (s8 !== peg$FAILED) {
                  peg$savedPos = s7;
                  s8 = peg$c227(s1, s5);
                }
                s7 = s8;
              }
              if (s6 !== peg$FAILED) {
                s7 = peg$parseExpression();
                if (s7 === peg$FAILED) {
                  s7 = peg$parseNode();
                }
                if (s7 !== peg$FAILED) {
                  s8 = [];
                  s9 = peg$currPos;
                  s10 = peg$parseSpace();
                  if (s10 !== peg$FAILED) {
                    peg$savedPos = s9;
                    s10 = peg$c228(s1, s5, s7);
                  }
                  s9 = s10;
                  while (s9 !== peg$FAILED) {
                    s8.push(s9);
                    s9 = peg$currPos;
                    s10 = peg$parseSpace();
                    if (s10 !== peg$FAILED) {
                      peg$savedPos = s9;
                      s10 = peg$c228(s1, s5, s7);
                    }
                    s9 = s10;
                  }
                  if (s8 !== peg$FAILED) {
                    peg$savedPos = s4;
                    s5 = peg$c229(s1, s5, s7);
                    s4 = s5;
                  } else {
                    peg$currPos = s4;
                    s4 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s4;
                  s4 = peg$FAILED;
                }
              } else {
                peg$currPos = s4;
                s4 = peg$FAILED;
              }
            } else {
              peg$currPos = s4;
              s4 = peg$FAILED;
            }
            if (s4 !== peg$FAILED) {
              while (s4 !== peg$FAILED) {
                s3.push(s4);
                s4 = peg$currPos;
                s5 = peg$parseCommonDelimiter();
                if (s5 !== peg$FAILED) {
                  s6 = [];
                  s7 = peg$currPos;
                  s8 = peg$parseSpace();
                  if (s8 !== peg$FAILED) {
                    peg$savedPos = s7;
                    s8 = peg$c227(s1, s5);
                  }
                  s7 = s8;
                  while (s7 !== peg$FAILED) {
                    s6.push(s7);
                    s7 = peg$currPos;
                    s8 = peg$parseSpace();
                    if (s8 !== peg$FAILED) {
                      peg$savedPos = s7;
                      s8 = peg$c227(s1, s5);
                    }
                    s7 = s8;
                  }
                  if (s6 !== peg$FAILED) {
                    s7 = peg$parseExpression();
                    if (s7 === peg$FAILED) {
                      s7 = peg$parseNode();
                    }
                    if (s7 !== peg$FAILED) {
                      s8 = [];
                      s9 = peg$currPos;
                      s10 = peg$parseSpace();
                      if (s10 !== peg$FAILED) {
                        peg$savedPos = s9;
                        s10 = peg$c228(s1, s5, s7);
                      }
                      s9 = s10;
                      while (s9 !== peg$FAILED) {
                        s8.push(s9);
                        s9 = peg$currPos;
                        s10 = peg$parseSpace();
                        if (s10 !== peg$FAILED) {
                          peg$savedPos = s9;
                          s10 = peg$c228(s1, s5, s7);
                        }
                        s9 = s10;
                      }
                      if (s8 !== peg$FAILED) {
                        peg$savedPos = s4;
                        s5 = peg$c229(s1, s5, s7);
                        s4 = s5;
                      } else {
                        peg$currPos = s4;
                        s4 = peg$FAILED;
                      }
                    } else {
                      peg$currPos = s4;
                      s4 = peg$FAILED;
                    }
                  } else {
                    peg$currPos = s4;
                    s4 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s4;
                  s4 = peg$FAILED;
                }
              }
            } else {
              s3 = peg$FAILED;
            }
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c230(s1, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c226);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parsePhraseExpression() {
        var s0, s1, s2, s3, s4, s5;

        var key = peg$currPos * 68 + 59,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseSequenceExpression();
        if (s1 === peg$FAILED) {
          s1 = peg$parseContainer();
          if (s1 === peg$FAILED) {
            s1 = peg$parseNode();
          }
        }
        if (s1 !== peg$FAILED) {
          s2 = [];
          s3 = peg$currPos;
          s4 = [];
          if (input.charCodeAt(peg$currPos) === 32) {
            s5 = peg$c14;
            peg$currPos++;
          } else {
            s5 = peg$FAILED;
            if (peg$silentFails === 0) {
              peg$fail(peg$c15);
            }
          }
          if (s5 === peg$FAILED) {
            if (peg$c12.test(input.charAt(peg$currPos))) {
              s5 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s5 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c13);
              }
            }
          }
          if (s5 !== peg$FAILED) {
            while (s5 !== peg$FAILED) {
              s4.push(s5);
              if (input.charCodeAt(peg$currPos) === 32) {
                s5 = peg$c14;
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c15);
                }
              }
              if (s5 === peg$FAILED) {
                if (peg$c12.test(input.charAt(peg$currPos))) {
                  s5 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s5 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c13);
                  }
                }
              }
            }
          } else {
            s4 = peg$FAILED;
          }
          if (s4 !== peg$FAILED) {
            s5 = peg$parseSequenceExpression();
            if (s5 === peg$FAILED) {
              s5 = peg$parseContainer();
              if (s5 === peg$FAILED) {
                s5 = peg$parseNode();
              }
            }
            if (s5 !== peg$FAILED) {
              peg$savedPos = s3;
              s4 = peg$c232(s1, s5);
              s3 = s4;
            } else {
              peg$currPos = s3;
              s3 = peg$FAILED;
            }
          } else {
            peg$currPos = s3;
            s3 = peg$FAILED;
          }
          if (s3 !== peg$FAILED) {
            while (s3 !== peg$FAILED) {
              s2.push(s3);
              s3 = peg$currPos;
              s4 = [];
              if (input.charCodeAt(peg$currPos) === 32) {
                s5 = peg$c14;
                peg$currPos++;
              } else {
                s5 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c15);
                }
              }
              if (s5 === peg$FAILED) {
                if (peg$c12.test(input.charAt(peg$currPos))) {
                  s5 = input.charAt(peg$currPos);
                  peg$currPos++;
                } else {
                  s5 = peg$FAILED;
                  if (peg$silentFails === 0) {
                    peg$fail(peg$c13);
                  }
                }
              }
              if (s5 !== peg$FAILED) {
                while (s5 !== peg$FAILED) {
                  s4.push(s5);
                  if (input.charCodeAt(peg$currPos) === 32) {
                    s5 = peg$c14;
                    peg$currPos++;
                  } else {
                    s5 = peg$FAILED;
                    if (peg$silentFails === 0) {
                      peg$fail(peg$c15);
                    }
                  }
                  if (s5 === peg$FAILED) {
                    if (peg$c12.test(input.charAt(peg$currPos))) {
                      s5 = input.charAt(peg$currPos);
                      peg$currPos++;
                    } else {
                      s5 = peg$FAILED;
                      if (peg$silentFails === 0) {
                        peg$fail(peg$c13);
                      }
                    }
                  }
                }
              } else {
                s4 = peg$FAILED;
              }
              if (s4 !== peg$FAILED) {
                s5 = peg$parseSequenceExpression();
                if (s5 === peg$FAILED) {
                  s5 = peg$parseContainer();
                  if (s5 === peg$FAILED) {
                    s5 = peg$parseNode();
                  }
                }
                if (s5 !== peg$FAILED) {
                  peg$savedPos = s3;
                  s4 = peg$c232(s1, s5);
                  s3 = s4;
                } else {
                  peg$currPos = s3;
                  s3 = peg$FAILED;
                }
              } else {
                peg$currPos = s3;
                s3 = peg$FAILED;
              }
            }
          } else {
            s2 = peg$FAILED;
          }
          if (s2 !== peg$FAILED) {
            peg$savedPos = s0;
            s1 = peg$c233(s1, s2);
            s0 = s1;
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c231);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseAggregationExpression() {
        var s0, s1, s2, s3, s4, s5, s6, s7, s8, s9;

        var key = peg$currPos * 68 + 60,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseContainer();
        if (s1 === peg$FAILED) {
          s1 = peg$parseNode();
        }
        if (s1 !== peg$FAILED) {
          s2 = [];
          s3 = peg$currPos;
          s4 = peg$parseSpace();
          if (s4 !== peg$FAILED) {
            peg$savedPos = s3;
            s4 = peg$c204(s1);
          }
          s3 = s4;
          while (s3 !== peg$FAILED) {
            s2.push(s3);
            s3 = peg$currPos;
            s4 = peg$parseSpace();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s3;
              s4 = peg$c204(s1);
            }
            s3 = s4;
          }
          if (s2 !== peg$FAILED) {
            s3 = [];
            s4 = peg$currPos;
            s5 = [];
            s6 = peg$currPos;
            s7 = peg$parseSpace();
            if (s7 !== peg$FAILED) {
              peg$savedPos = s6;
              s7 = peg$c204(s1);
            }
            s6 = s7;
            while (s6 !== peg$FAILED) {
              s5.push(s6);
              s6 = peg$currPos;
              s7 = peg$parseSpace();
              if (s7 !== peg$FAILED) {
                peg$savedPos = s6;
                s7 = peg$c204(s1);
              }
              s6 = s7;
            }
            if (s5 !== peg$FAILED) {
              s6 = peg$parseAggregationOperator();
              if (s6 !== peg$FAILED) {
                s7 = [];
                s8 = peg$currPos;
                s9 = peg$parseSpace();
                if (s9 !== peg$FAILED) {
                  peg$savedPos = s8;
                  s9 = peg$c227(s1, s6);
                }
                s8 = s9;
                while (s8 !== peg$FAILED) {
                  s7.push(s8);
                  s8 = peg$currPos;
                  s9 = peg$parseSpace();
                  if (s9 !== peg$FAILED) {
                    peg$savedPos = s8;
                    s9 = peg$c227(s1, s6);
                  }
                  s8 = s9;
                }
                if (s7 !== peg$FAILED) {
                  s8 = peg$parseExpression();
                  if (s8 === peg$FAILED) {
                    s8 = peg$parseNode();
                  }
                  if (s8 !== peg$FAILED) {
                    peg$savedPos = s4;
                    s5 = peg$c235(s1, s6, s8);
                    s4 = s5;
                  } else {
                    peg$currPos = s4;
                    s4 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s4;
                  s4 = peg$FAILED;
                }
              } else {
                peg$currPos = s4;
                s4 = peg$FAILED;
              }
            } else {
              peg$currPos = s4;
              s4 = peg$FAILED;
            }
            if (s4 !== peg$FAILED) {
              while (s4 !== peg$FAILED) {
                s3.push(s4);
                s4 = peg$currPos;
                s5 = [];
                s6 = peg$currPos;
                s7 = peg$parseSpace();
                if (s7 !== peg$FAILED) {
                  peg$savedPos = s6;
                  s7 = peg$c204(s1);
                }
                s6 = s7;
                while (s6 !== peg$FAILED) {
                  s5.push(s6);
                  s6 = peg$currPos;
                  s7 = peg$parseSpace();
                  if (s7 !== peg$FAILED) {
                    peg$savedPos = s6;
                    s7 = peg$c204(s1);
                  }
                  s6 = s7;
                }
                if (s5 !== peg$FAILED) {
                  s6 = peg$parseAggregationOperator();
                  if (s6 !== peg$FAILED) {
                    s7 = [];
                    s8 = peg$currPos;
                    s9 = peg$parseSpace();
                    if (s9 !== peg$FAILED) {
                      peg$savedPos = s8;
                      s9 = peg$c227(s1, s6);
                    }
                    s8 = s9;
                    while (s8 !== peg$FAILED) {
                      s7.push(s8);
                      s8 = peg$currPos;
                      s9 = peg$parseSpace();
                      if (s9 !== peg$FAILED) {
                        peg$savedPos = s8;
                        s9 = peg$c227(s1, s6);
                      }
                      s8 = s9;
                    }
                    if (s7 !== peg$FAILED) {
                      s8 = peg$parseExpression();
                      if (s8 === peg$FAILED) {
                        s8 = peg$parseNode();
                      }
                      if (s8 !== peg$FAILED) {
                        peg$savedPos = s4;
                        s5 = peg$c235(s1, s6, s8);
                        s4 = s5;
                      } else {
                        peg$currPos = s4;
                        s4 = peg$FAILED;
                      }
                    } else {
                      peg$currPos = s4;
                      s4 = peg$FAILED;
                    }
                  } else {
                    peg$currPos = s4;
                    s4 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s4;
                  s4 = peg$FAILED;
                }
              }
            } else {
              s3 = peg$FAILED;
            }
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c236(s1, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c234);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseInfixedBindingExpression() {
        var s0, s1, s2, s3, s4, s5, s6, s7, s8, s9;

        var key = peg$currPos * 68 + 61,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseContainer();
        if (s1 === peg$FAILED) {
          s1 = peg$parseNode();
          if (s1 === peg$FAILED) {
            s1 = peg$parseChannelOperator();
          }
        }
        if (s1 !== peg$FAILED) {
          s2 = [];
          s3 = peg$currPos;
          s4 = peg$parseSpace();
          if (s4 !== peg$FAILED) {
            peg$savedPos = s3;
            s4 = peg$c204(s1);
          }
          s3 = s4;
          while (s3 !== peg$FAILED) {
            s2.push(s3);
            s3 = peg$currPos;
            s4 = peg$parseSpace();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s3;
              s4 = peg$c204(s1);
            }
            s3 = s4;
          }
          if (s2 !== peg$FAILED) {
            s3 = [];
            s4 = peg$currPos;
            s5 = [];
            s6 = peg$currPos;
            s7 = peg$parseSpace();
            if (s7 !== peg$FAILED) {
              peg$savedPos = s6;
              s7 = peg$c204(s1);
            }
            s6 = s7;
            while (s6 !== peg$FAILED) {
              s5.push(s6);
              s6 = peg$currPos;
              s7 = peg$parseSpace();
              if (s7 !== peg$FAILED) {
                peg$savedPos = s6;
                s7 = peg$c204(s1);
              }
              s6 = s7;
            }
            if (s5 !== peg$FAILED) {
              s6 = peg$parseBindingOperator();
              if (s6 !== peg$FAILED) {
                s7 = [];
                s8 = peg$currPos;
                s9 = peg$parseSpace();
                if (s9 !== peg$FAILED) {
                  peg$savedPos = s8;
                  s9 = peg$c227(s1, s6);
                }
                s8 = s9;
                while (s8 !== peg$FAILED) {
                  s7.push(s8);
                  s8 = peg$currPos;
                  s9 = peg$parseSpace();
                  if (s9 !== peg$FAILED) {
                    peg$savedPos = s8;
                    s9 = peg$c227(s1, s6);
                  }
                  s8 = s9;
                }
                if (s7 !== peg$FAILED) {
                  s8 = peg$parseExpression();
                  if (s8 === peg$FAILED) {
                    s8 = peg$parseNode();
                  }
                  if (s8 !== peg$FAILED) {
                    peg$savedPos = s4;
                    s5 = peg$c238(s1, s6, s8);
                    s4 = s5;
                  } else {
                    peg$currPos = s4;
                    s4 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s4;
                  s4 = peg$FAILED;
                }
              } else {
                peg$currPos = s4;
                s4 = peg$FAILED;
              }
            } else {
              peg$currPos = s4;
              s4 = peg$FAILED;
            }
            if (s4 !== peg$FAILED) {
              while (s4 !== peg$FAILED) {
                s3.push(s4);
                s4 = peg$currPos;
                s5 = [];
                s6 = peg$currPos;
                s7 = peg$parseSpace();
                if (s7 !== peg$FAILED) {
                  peg$savedPos = s6;
                  s7 = peg$c204(s1);
                }
                s6 = s7;
                while (s6 !== peg$FAILED) {
                  s5.push(s6);
                  s6 = peg$currPos;
                  s7 = peg$parseSpace();
                  if (s7 !== peg$FAILED) {
                    peg$savedPos = s6;
                    s7 = peg$c204(s1);
                  }
                  s6 = s7;
                }
                if (s5 !== peg$FAILED) {
                  s6 = peg$parseBindingOperator();
                  if (s6 !== peg$FAILED) {
                    s7 = [];
                    s8 = peg$currPos;
                    s9 = peg$parseSpace();
                    if (s9 !== peg$FAILED) {
                      peg$savedPos = s8;
                      s9 = peg$c227(s1, s6);
                    }
                    s8 = s9;
                    while (s8 !== peg$FAILED) {
                      s7.push(s8);
                      s8 = peg$currPos;
                      s9 = peg$parseSpace();
                      if (s9 !== peg$FAILED) {
                        peg$savedPos = s8;
                        s9 = peg$c227(s1, s6);
                      }
                      s8 = s9;
                    }
                    if (s7 !== peg$FAILED) {
                      s8 = peg$parseExpression();
                      if (s8 === peg$FAILED) {
                        s8 = peg$parseNode();
                      }
                      if (s8 !== peg$FAILED) {
                        peg$savedPos = s4;
                        s5 = peg$c238(s1, s6, s8);
                        s4 = s5;
                      } else {
                        peg$currPos = s4;
                        s4 = peg$FAILED;
                      }
                    } else {
                      peg$currPos = s4;
                      s4 = peg$FAILED;
                    }
                  } else {
                    peg$currPos = s4;
                    s4 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s4;
                  s4 = peg$FAILED;
                }
              }
            } else {
              s3 = peg$FAILED;
            }
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c239(s1, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c237);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseInfixedReductionExpression() {
        var s0, s1, s2, s3, s4, s5, s6, s7, s8, s9;

        var key = peg$currPos * 68 + 62,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseContainer();
        if (s1 === peg$FAILED) {
          s1 = peg$parseNode();
        }
        if (s1 !== peg$FAILED) {
          s2 = [];
          s3 = peg$currPos;
          s4 = peg$parseSpace();
          if (s4 !== peg$FAILED) {
            peg$savedPos = s3;
            s4 = peg$c204(s1);
          }
          s3 = s4;
          while (s3 !== peg$FAILED) {
            s2.push(s3);
            s3 = peg$currPos;
            s4 = peg$parseSpace();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s3;
              s4 = peg$c204(s1);
            }
            s3 = s4;
          }
          if (s2 !== peg$FAILED) {
            s3 = [];
            s4 = peg$currPos;
            s5 = [];
            s6 = peg$currPos;
            s7 = peg$parseSpace();
            if (s7 !== peg$FAILED) {
              peg$savedPos = s6;
              s7 = peg$c204(s1);
            }
            s6 = s7;
            while (s6 !== peg$FAILED) {
              s5.push(s6);
              s6 = peg$currPos;
              s7 = peg$parseSpace();
              if (s7 !== peg$FAILED) {
                peg$savedPos = s6;
                s7 = peg$c204(s1);
              }
              s6 = s7;
            }
            if (s5 !== peg$FAILED) {
              s6 = peg$parseReductionOperator();
              if (s6 !== peg$FAILED) {
                s7 = [];
                s8 = peg$currPos;
                s9 = peg$parseSpace();
                if (s9 !== peg$FAILED) {
                  peg$savedPos = s8;
                  s9 = peg$c227(s1, s6);
                }
                s8 = s9;
                while (s8 !== peg$FAILED) {
                  s7.push(s8);
                  s8 = peg$currPos;
                  s9 = peg$parseSpace();
                  if (s9 !== peg$FAILED) {
                    peg$savedPos = s8;
                    s9 = peg$c227(s1, s6);
                  }
                  s8 = s9;
                }
                if (s7 !== peg$FAILED) {
                  s8 = peg$parseExpression();
                  if (s8 === peg$FAILED) {
                    s8 = peg$parseNode();
                  }
                  if (s8 !== peg$FAILED) {
                    peg$savedPos = s4;
                    s5 = peg$c241(s1, s6, s8);
                    s4 = s5;
                  } else {
                    peg$currPos = s4;
                    s4 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s4;
                  s4 = peg$FAILED;
                }
              } else {
                peg$currPos = s4;
                s4 = peg$FAILED;
              }
            } else {
              peg$currPos = s4;
              s4 = peg$FAILED;
            }
            if (s4 !== peg$FAILED) {
              while (s4 !== peg$FAILED) {
                s3.push(s4);
                s4 = peg$currPos;
                s5 = [];
                s6 = peg$currPos;
                s7 = peg$parseSpace();
                if (s7 !== peg$FAILED) {
                  peg$savedPos = s6;
                  s7 = peg$c204(s1);
                }
                s6 = s7;
                while (s6 !== peg$FAILED) {
                  s5.push(s6);
                  s6 = peg$currPos;
                  s7 = peg$parseSpace();
                  if (s7 !== peg$FAILED) {
                    peg$savedPos = s6;
                    s7 = peg$c204(s1);
                  }
                  s6 = s7;
                }
                if (s5 !== peg$FAILED) {
                  s6 = peg$parseReductionOperator();
                  if (s6 !== peg$FAILED) {
                    s7 = [];
                    s8 = peg$currPos;
                    s9 = peg$parseSpace();
                    if (s9 !== peg$FAILED) {
                      peg$savedPos = s8;
                      s9 = peg$c227(s1, s6);
                    }
                    s8 = s9;
                    while (s8 !== peg$FAILED) {
                      s7.push(s8);
                      s8 = peg$currPos;
                      s9 = peg$parseSpace();
                      if (s9 !== peg$FAILED) {
                        peg$savedPos = s8;
                        s9 = peg$c227(s1, s6);
                      }
                      s8 = s9;
                    }
                    if (s7 !== peg$FAILED) {
                      s8 = peg$parseExpression();
                      if (s8 === peg$FAILED) {
                        s8 = peg$parseNode();
                      }
                      if (s8 !== peg$FAILED) {
                        peg$savedPos = s4;
                        s5 = peg$c241(s1, s6, s8);
                        s4 = s5;
                      } else {
                        peg$currPos = s4;
                        s4 = peg$FAILED;
                      }
                    } else {
                      peg$currPos = s4;
                      s4 = peg$FAILED;
                    }
                  } else {
                    peg$currPos = s4;
                    s4 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s4;
                  s4 = peg$FAILED;
                }
              }
            } else {
              s3 = peg$FAILED;
            }
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c242(s1, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c240);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseInfixedTransformationExpression() {
        var s0, s1, s2, s3, s4, s5, s6, s7, s8, s9;

        var key = peg$currPos * 68 + 63,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseContainer();
        if (s1 === peg$FAILED) {
          s1 = peg$parseNode();
          if (s1 === peg$FAILED) {
            s1 = peg$parseChannelOperator();
          }
        }
        if (s1 !== peg$FAILED) {
          s2 = [];
          s3 = peg$currPos;
          s4 = peg$parseSpace();
          if (s4 !== peg$FAILED) {
            peg$savedPos = s3;
            s4 = peg$c204(s1);
          }
          s3 = s4;
          while (s3 !== peg$FAILED) {
            s2.push(s3);
            s3 = peg$currPos;
            s4 = peg$parseSpace();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s3;
              s4 = peg$c204(s1);
            }
            s3 = s4;
          }
          if (s2 !== peg$FAILED) {
            s3 = [];
            s4 = peg$currPos;
            s5 = [];
            s6 = peg$currPos;
            s7 = peg$parseSpace();
            if (s7 !== peg$FAILED) {
              peg$savedPos = s6;
              s7 = peg$c204(s1);
            }
            s6 = s7;
            while (s6 !== peg$FAILED) {
              s5.push(s6);
              s6 = peg$currPos;
              s7 = peg$parseSpace();
              if (s7 !== peg$FAILED) {
                peg$savedPos = s6;
                s7 = peg$c204(s1);
              }
              s6 = s7;
            }
            if (s5 !== peg$FAILED) {
              s6 = peg$parseTransformationOperator();
              if (s6 !== peg$FAILED) {
                s7 = [];
                s8 = peg$currPos;
                s9 = peg$parseSpace();
                if (s9 !== peg$FAILED) {
                  peg$savedPos = s8;
                  s9 = peg$c227(s1, s6);
                }
                s8 = s9;
                while (s8 !== peg$FAILED) {
                  s7.push(s8);
                  s8 = peg$currPos;
                  s9 = peg$parseSpace();
                  if (s9 !== peg$FAILED) {
                    peg$savedPos = s8;
                    s9 = peg$c227(s1, s6);
                  }
                  s8 = s9;
                }
                if (s7 !== peg$FAILED) {
                  s8 = peg$parseExpression();
                  if (s8 === peg$FAILED) {
                    s8 = peg$parseContainer();
                    if (s8 === peg$FAILED) {
                      s8 = peg$parseNode();
                    }
                  }
                  if (s8 !== peg$FAILED) {
                    peg$savedPos = s4;
                    s5 = peg$c244(s1, s6, s8);
                    s4 = s5;
                  } else {
                    peg$currPos = s4;
                    s4 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s4;
                  s4 = peg$FAILED;
                }
              } else {
                peg$currPos = s4;
                s4 = peg$FAILED;
              }
            } else {
              peg$currPos = s4;
              s4 = peg$FAILED;
            }
            if (s4 !== peg$FAILED) {
              while (s4 !== peg$FAILED) {
                s3.push(s4);
                s4 = peg$currPos;
                s5 = [];
                s6 = peg$currPos;
                s7 = peg$parseSpace();
                if (s7 !== peg$FAILED) {
                  peg$savedPos = s6;
                  s7 = peg$c204(s1);
                }
                s6 = s7;
                while (s6 !== peg$FAILED) {
                  s5.push(s6);
                  s6 = peg$currPos;
                  s7 = peg$parseSpace();
                  if (s7 !== peg$FAILED) {
                    peg$savedPos = s6;
                    s7 = peg$c204(s1);
                  }
                  s6 = s7;
                }
                if (s5 !== peg$FAILED) {
                  s6 = peg$parseTransformationOperator();
                  if (s6 !== peg$FAILED) {
                    s7 = [];
                    s8 = peg$currPos;
                    s9 = peg$parseSpace();
                    if (s9 !== peg$FAILED) {
                      peg$savedPos = s8;
                      s9 = peg$c227(s1, s6);
                    }
                    s8 = s9;
                    while (s8 !== peg$FAILED) {
                      s7.push(s8);
                      s8 = peg$currPos;
                      s9 = peg$parseSpace();
                      if (s9 !== peg$FAILED) {
                        peg$savedPos = s8;
                        s9 = peg$c227(s1, s6);
                      }
                      s8 = s9;
                    }
                    if (s7 !== peg$FAILED) {
                      s8 = peg$parseExpression();
                      if (s8 === peg$FAILED) {
                        s8 = peg$parseContainer();
                        if (s8 === peg$FAILED) {
                          s8 = peg$parseNode();
                        }
                      }
                      if (s8 !== peg$FAILED) {
                        peg$savedPos = s4;
                        s5 = peg$c244(s1, s6, s8);
                        s4 = s5;
                      } else {
                        peg$currPos = s4;
                        s4 = peg$FAILED;
                      }
                    } else {
                      peg$currPos = s4;
                      s4 = peg$FAILED;
                    }
                  } else {
                    peg$currPos = s4;
                    s4 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s4;
                  s4 = peg$FAILED;
                }
              }
            } else {
              s3 = peg$FAILED;
            }
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c245(s1, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c243);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseInfixedRangeExpression() {
        var s0, s1, s2, s3, s4, s5, s6, s7, s8, s9;

        var key = peg$currPos * 68 + 64,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseContainer();
        if (s1 === peg$FAILED) {
          s1 = peg$parseNode();
          if (s1 === peg$FAILED) {
            s1 = peg$parseChannelOperator();
          }
        }
        if (s1 !== peg$FAILED) {
          s2 = [];
          s3 = peg$currPos;
          s4 = peg$parseSpace();
          if (s4 !== peg$FAILED) {
            peg$savedPos = s3;
            s4 = peg$c204(s1);
          }
          s3 = s4;
          while (s3 !== peg$FAILED) {
            s2.push(s3);
            s3 = peg$currPos;
            s4 = peg$parseSpace();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s3;
              s4 = peg$c204(s1);
            }
            s3 = s4;
          }
          if (s2 !== peg$FAILED) {
            s3 = [];
            s4 = peg$currPos;
            s5 = [];
            s6 = peg$currPos;
            s7 = peg$parseSpace();
            if (s7 !== peg$FAILED) {
              peg$savedPos = s6;
              s7 = peg$c204(s1);
            }
            s6 = s7;
            while (s6 !== peg$FAILED) {
              s5.push(s6);
              s6 = peg$currPos;
              s7 = peg$parseSpace();
              if (s7 !== peg$FAILED) {
                peg$savedPos = s6;
                s7 = peg$c204(s1);
              }
              s6 = s7;
            }
            if (s5 !== peg$FAILED) {
              s6 = peg$parseRangeOperator();
              if (s6 !== peg$FAILED) {
                s7 = [];
                s8 = peg$currPos;
                s9 = peg$parseSpace();
                if (s9 !== peg$FAILED) {
                  peg$savedPos = s8;
                  s9 = peg$c227(s1, s6);
                }
                s8 = s9;
                while (s8 !== peg$FAILED) {
                  s7.push(s8);
                  s8 = peg$currPos;
                  s9 = peg$parseSpace();
                  if (s9 !== peg$FAILED) {
                    peg$savedPos = s8;
                    s9 = peg$c227(s1, s6);
                  }
                  s8 = s9;
                }
                if (s7 !== peg$FAILED) {
                  s8 = peg$parseExpression();
                  if (s8 === peg$FAILED) {
                    s8 = peg$parseNode();
                  }
                  if (s8 !== peg$FAILED) {
                    peg$savedPos = s4;
                    s5 = peg$c247(s1, s6, s8);
                    s4 = s5;
                  } else {
                    peg$currPos = s4;
                    s4 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s4;
                  s4 = peg$FAILED;
                }
              } else {
                peg$currPos = s4;
                s4 = peg$FAILED;
              }
            } else {
              peg$currPos = s4;
              s4 = peg$FAILED;
            }
            if (s4 !== peg$FAILED) {
              while (s4 !== peg$FAILED) {
                s3.push(s4);
                s4 = peg$currPos;
                s5 = [];
                s6 = peg$currPos;
                s7 = peg$parseSpace();
                if (s7 !== peg$FAILED) {
                  peg$savedPos = s6;
                  s7 = peg$c204(s1);
                }
                s6 = s7;
                while (s6 !== peg$FAILED) {
                  s5.push(s6);
                  s6 = peg$currPos;
                  s7 = peg$parseSpace();
                  if (s7 !== peg$FAILED) {
                    peg$savedPos = s6;
                    s7 = peg$c204(s1);
                  }
                  s6 = s7;
                }
                if (s5 !== peg$FAILED) {
                  s6 = peg$parseRangeOperator();
                  if (s6 !== peg$FAILED) {
                    s7 = [];
                    s8 = peg$currPos;
                    s9 = peg$parseSpace();
                    if (s9 !== peg$FAILED) {
                      peg$savedPos = s8;
                      s9 = peg$c227(s1, s6);
                    }
                    s8 = s9;
                    while (s8 !== peg$FAILED) {
                      s7.push(s8);
                      s8 = peg$currPos;
                      s9 = peg$parseSpace();
                      if (s9 !== peg$FAILED) {
                        peg$savedPos = s8;
                        s9 = peg$c227(s1, s6);
                      }
                      s8 = s9;
                    }
                    if (s7 !== peg$FAILED) {
                      s8 = peg$parseExpression();
                      if (s8 === peg$FAILED) {
                        s8 = peg$parseNode();
                      }
                      if (s8 !== peg$FAILED) {
                        peg$savedPos = s4;
                        s5 = peg$c247(s1, s6, s8);
                        s4 = s5;
                      } else {
                        peg$currPos = s4;
                        s4 = peg$FAILED;
                      }
                    } else {
                      peg$currPos = s4;
                      s4 = peg$FAILED;
                    }
                  } else {
                    peg$currPos = s4;
                    s4 = peg$FAILED;
                  }
                } else {
                  peg$currPos = s4;
                  s4 = peg$FAILED;
                }
              }
            } else {
              s3 = peg$FAILED;
            }
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c248(s1, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c246);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parseInfixExpression() {
        var s0, s1;

        var key = peg$currPos * 68 + 65,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$parseCommonExpression();
        if (s0 === peg$FAILED) {
          s0 = peg$parsePhraseExpression();
          if (s0 === peg$FAILED) {
            s0 = peg$parseInfixedBindingExpression();
            if (s0 === peg$FAILED) {
              s0 = peg$parseInfixedTransformationExpression();
              if (s0 === peg$FAILED) {
                s0 = peg$parseAggregationExpression();
                if (s0 === peg$FAILED) {
                  s0 = peg$parseInfixedReductionExpression();
                  if (s0 === peg$FAILED) {
                    s0 = peg$parseInfixedRangeExpression();
                  }
                }
              }
            }
          }
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c249);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parsePrefixExpression() {
        var s0, s1, s2, s3, s4;

        var key = peg$currPos * 68 + 66,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = peg$parseSpreadOperator();
        if (s1 === peg$FAILED) {
          s1 = peg$parseRangeOperator();
          if (s1 === peg$FAILED) {
            s1 = peg$parseDescentOperator();
            if (s1 === peg$FAILED) {
              s1 = peg$parseDirectionOperator();
              if (s1 === peg$FAILED) {
                s1 = peg$parseAggregationOperator();
                if (s1 === peg$FAILED) {
                  s1 = peg$parseAscentOperator();
                  if (s1 === peg$FAILED) {
                    s1 = peg$parseBindingOperator();
                    if (s1 === peg$FAILED) {
                      s1 = peg$parseBranchOperator();
                      if (s1 === peg$FAILED) {
                        s1 = peg$parseChannelOperator();
                        if (s1 === peg$FAILED) {
                          s1 = peg$parseEvaluationOperator();
                          if (s1 === peg$FAILED) {
                            s1 = peg$parseInvocationOperator();
                            if (s1 === peg$FAILED) {
                              s1 = peg$parsePerformanceOperator();
                              if (s1 === peg$FAILED) {
                                s1 = peg$parsePerspectiveOperator();
                                if (s1 === peg$FAILED) {
                                  s1 = peg$parseReductionOperator();
                                  if (s1 === peg$FAILED) {
                                    s1 = peg$parseReferenceOperator();
                                    if (s1 === peg$FAILED) {
                                      s1 = peg$parseValueOperator();
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        }
        if (s1 !== peg$FAILED) {
          s2 = [];
          s3 = peg$currPos;
          s4 = peg$parseSpace();
          if (s4 !== peg$FAILED) {
            peg$savedPos = s3;
            s4 = peg$c251(s1);
          }
          s3 = s4;
          if (s3 === peg$FAILED) {
            if (peg$c2.test(input.charAt(peg$currPos))) {
              s3 = input.charAt(peg$currPos);
              peg$currPos++;
            } else {
              s3 = peg$FAILED;
              if (peg$silentFails === 0) {
                peg$fail(peg$c3);
              }
            }
          }
          while (s3 !== peg$FAILED) {
            s2.push(s3);
            s3 = peg$currPos;
            s4 = peg$parseSpace();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s3;
              s4 = peg$c251(s1);
            }
            s3 = s4;
            if (s3 === peg$FAILED) {
              if (peg$c2.test(input.charAt(peg$currPos))) {
                s3 = input.charAt(peg$currPos);
                peg$currPos++;
              } else {
                s3 = peg$FAILED;
                if (peg$silentFails === 0) {
                  peg$fail(peg$c3);
                }
              }
            }
          }
          if (s2 !== peg$FAILED) {
            s3 = peg$parseExpression();
            if (s3 === peg$FAILED) {
              s3 = peg$parseNode();
            }
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c252(s1, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c250);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function peg$parsePostfixExpression() {
        var s0, s1, s2, s3, s4, s5, s6, s7;

        var key = peg$currPos * 68 + 67,
          cached = peg$resultsCache[key];

        if (cached) {
          peg$currPos = cached.nextPos;

          return cached.result;
        }

        peg$silentFails++;
        s0 = peg$currPos;
        s1 = [];
        s2 = peg$currPos;
        s3 = peg$parsePrefixExpression();
        if (s3 === peg$FAILED) {
          s3 = peg$parseInfixExpression();
          if (s3 === peg$FAILED) {
            s3 = peg$parseNode();
          }
        }
        if (s3 !== peg$FAILED) {
          s4 = peg$parseNodeDelimiter();
          if (s4 !== peg$FAILED) {
            s5 = [];
            s6 = peg$currPos;
            s7 = peg$parseSpace();
            if (s7 !== peg$FAILED) {
              peg$savedPos = s6;
              s7 = peg$c254(s3);
            }
            s6 = s7;
            while (s6 !== peg$FAILED) {
              s5.push(s6);
              s6 = peg$currPos;
              s7 = peg$parseSpace();
              if (s7 !== peg$FAILED) {
                peg$savedPos = s6;
                s7 = peg$c254(s3);
              }
              s6 = s7;
            }
            if (s5 !== peg$FAILED) {
              peg$savedPos = s2;
              s3 = peg$c255(s3);
              s2 = s3;
            } else {
              peg$currPos = s2;
              s2 = peg$FAILED;
            }
          } else {
            peg$currPos = s2;
            s2 = peg$FAILED;
          }
        } else {
          peg$currPos = s2;
          s2 = peg$FAILED;
        }
        while (s2 !== peg$FAILED) {
          s1.push(s2);
          s2 = peg$currPos;
          s3 = peg$parsePrefixExpression();
          if (s3 === peg$FAILED) {
            s3 = peg$parseInfixExpression();
            if (s3 === peg$FAILED) {
              s3 = peg$parseNode();
            }
          }
          if (s3 !== peg$FAILED) {
            s4 = peg$parseNodeDelimiter();
            if (s4 !== peg$FAILED) {
              s5 = [];
              s6 = peg$currPos;
              s7 = peg$parseSpace();
              if (s7 !== peg$FAILED) {
                peg$savedPos = s6;
                s7 = peg$c254(s3);
              }
              s6 = s7;
              while (s6 !== peg$FAILED) {
                s5.push(s6);
                s6 = peg$currPos;
                s7 = peg$parseSpace();
                if (s7 !== peg$FAILED) {
                  peg$savedPos = s6;
                  s7 = peg$c254(s3);
                }
                s6 = s7;
              }
              if (s5 !== peg$FAILED) {
                peg$savedPos = s2;
                s3 = peg$c255(s3);
                s2 = s3;
              } else {
                peg$currPos = s2;
                s2 = peg$FAILED;
              }
            } else {
              peg$currPos = s2;
              s2 = peg$FAILED;
            }
          } else {
            peg$currPos = s2;
            s2 = peg$FAILED;
          }
        }
        if (s1 === peg$FAILED) {
          s1 = peg$parsePrefixExpression();
          if (s1 === peg$FAILED) {
            s1 = peg$parseInfixExpression();
            if (s1 === peg$FAILED) {
              s1 = peg$parseNode();
            }
          }
        }
        if (s1 !== peg$FAILED) {
          s2 = [];
          s3 = peg$currPos;
          s4 = peg$parseSpace();
          if (s4 !== peg$FAILED) {
            peg$savedPos = s3;
            s4 = peg$c256(s1);
          }
          s3 = s4;
          while (s3 !== peg$FAILED) {
            s2.push(s3);
            s3 = peg$currPos;
            s4 = peg$parseSpace();
            if (s4 !== peg$FAILED) {
              peg$savedPos = s3;
              s4 = peg$c256(s1);
            }
            s3 = s4;
          }
          if (s2 !== peg$FAILED) {
            s3 = peg$parseOperator();
            if (s3 !== peg$FAILED) {
              peg$savedPos = s0;
              s1 = peg$c257(s1, s3);
              s0 = s1;
            } else {
              peg$currPos = s0;
              s0 = peg$FAILED;
            }
          } else {
            peg$currPos = s0;
            s0 = peg$FAILED;
          }
        } else {
          peg$currPos = s0;
          s0 = peg$FAILED;
        }
        peg$silentFails--;
        if (s0 === peg$FAILED) {
          s1 = peg$FAILED;
          if (peg$silentFails === 0) {
            peg$fail(peg$c253);
          }
        }

        peg$resultsCache[key] = { nextPos: peg$currPos, result: s0 };

        return s0;
      }

      function spwHead() {
        var _cache = new Map();
        /**
         * add the usual things to a node, remove missteps
         * @param node
         */
        function normalize(
          // @ts-ignore
          node,
        ) {
          return Object.assign(
            {
              src: text(),
              srcloc: location(),
            },
            Object.fromEntries(
              Object.entries(node).filter(function (e) {
                var k = e[0];
                var v = e[1];
                return k === 'key' ? true : v !== undefined;
              }),
            ),
          );
        }
        /**
         * Convert a node to a {@see Construct } Construct initializer
         * @param node
         */
        function toConstruct(
          // @ts-ignore
          node,
        ) {
          if (typeof location === 'undefined') return;
          if (!node.kind) throw new Error('No node kind specified');
          var cacheKey = JSON.stringify(location());
          if (_cache.has(cacheKey)) return _cache.get(cacheKey);
          var out = normalize(node);
          _cache.set(cacheKey, out);
          return out;
        }
        /** Actions **/
        var constructs = {
          space: function () {
            return toConstruct({ kind: 'space' });
          },
        };
        return {
          toConstruct: toConstruct,
          constructs: constructs,
        };
      }
      var head = spwHead();
      var toConstruct = head.toConstruct;
      var constructs = head.constructs;

      peg$result = peg$startRuleFunction();

      if (peg$result !== peg$FAILED && peg$currPos === input.length) {
        return peg$result;
      } else {
        if (peg$result !== peg$FAILED && peg$currPos < input.length) {
          peg$fail(peg$endExpectation());
        }

        throw peg$buildStructuredError(
          peg$maxFailExpected,
          peg$maxFailPos < input.length ? input.charAt(peg$maxFailPos) : null,
          peg$maxFailPos < input.length
            ? peg$computeLocation(peg$maxFailPos, peg$maxFailPos + 1)
            : peg$computeLocation(peg$maxFailPos, peg$maxFailPos),
        );
      }
    }

    return {
      SyntaxError: peg$SyntaxError,
      parse: peg$parse,
    };
  })();
export default generatedParser;
export { generatedParser as spwParser };
